...
class FinanceChart : public MultiChart
{
        ...
        DoubleArray openData, DoubleArray closeData, DoubleArray volData, int extraPoints)
    {
		...
		while ((unitIndex > 0) && (maxVol < qPow(1000.0, unitIndex))) {
            unitIndex = unitIndex - 1;
        }

		deepCopy(m_volData, ArrayMath(volData).div(qPow(1000.0, unitIndex)));
        ...
        int symbolType, int symbolSize, int fillColor, int edgeColor)
    {
        ...
        ret->setLineWidth(0);
        ret->addDataSet(DoubleArray(psar, m_lowData.len))
			->setDataSymbol(symbolType, symbolSize, fillColor, edgeColor);
        ...
        int fillColor, const char* name)
    {
        ...
        LineLayer *uLayer = m_mainChart->addLineLayer(upperLine, lineColor, name);
        LineLayer *lLayer = m_mainChart->addLineLayer(lowerLine, lineColor);
    ...
    {		
        ...
        if (i >= 0) {
            sprintf(buffer, "%s: %s%s", label.c_str(), formatValue(m_volData[i], m_generalFormat.c_str()), 
				m_volUnit.c_str());
            label = buffer;
 			const_cast<double *>(closeChangeData.data)[0] = 0;
        }
    ...
    /// <returns>The XYChart object representing the chart created.</returns>
    public: XYChart* addCCI(int height, int period, int color, double range, int upColor,
        int downColor)
    {
        //typical price
