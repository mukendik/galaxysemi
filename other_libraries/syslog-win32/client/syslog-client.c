...
static void init_logger_addr()
{
    ...
    else
    {
        /* relative path */
        char *q;
        // __argv[] is not accessible and provokes a crash when syslog-client.c is inside a win32 dll
        /* // We could bypass but we could need to use the syslog.host
        if (!__argv)
            goto use_default;
        */

        //strcpy( pathname, __argv[0] ); // let s try something smarter
        strcpy( pathname, __argv?__argv[0]:"." );
    ...
    //printf("syslog-client.c: opening '%s'\n", pathname);
...
use_default:
    target=getenv((char*)"GS_SYSLOG_TARGET");
    if (target)
    {
        struct hostent *he;
        he=gethostbyname(target);
        if (he)
            sa_logger.sin_addr.S_un.S_addr = *(u_long *)he->h_addr_list[0];
        else
...
#define GET_DEFINE_STRING(x) GETSTRING(x)
...
#ifdef MODULE
    openlog( GET_DEFINE_STRING(MODULE), 0, pri & LOG_FACMASK ); //openlog( NULL, 0, pri & LOG_FACMASK );
#else
    openlog( "", 0, pri & LOG_FACMASK ); //openlog( NULL, 0, pri & LOG_FACMASK );
#endif
    if( !initialized )
        return;
