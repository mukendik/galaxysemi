...
static const char *getlogname (void)
{
  ...
  if (!GetModuleFileName(NULL, (LPWSTR) path,sizeof(path)) || !path[0])
     goto unknown;
...
{
  HMODULE mod = LoadLibrary ((LPCWSTR)"iphlpapi.dll");
  BOOL    rc = TRUE;
  ...
  if (mod)
  {
    ...
    //(void*)SendARP = GetProcAddress(mod, (LPCSTR)"SendARP");
    SendARP = GetProcAddress(mod, (LPCSTR)"SendARP");
    if (SendARP)
    {
...
static BOOL get_netmask (DWORD *mask)
{
  ...
  if (mod)
  {
    ...
    GetAdaptersInfo = GetProcAddress (mod, "GetAdaptersInfo");
