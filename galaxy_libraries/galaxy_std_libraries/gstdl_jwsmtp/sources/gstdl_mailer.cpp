...
// With Sun's C++ compiler and GNU g++ compiler, need to declare these externals!
...
mailer::mailer(const char* TOaddress, const char* FROMaddress,
			   ...
			   bool MXLookup, bool bTraceEnabled, const char* szTraceFile):
      type(LOGIN),
		subject(Subject),
...
mailer::mailer(bool MXLookup, unsigned short Port, bool bTraceEnabled, const char* szTraceFile):port(htons(Port)), lookupMXRecord(MXLookup)
{
	initNetworking(); // in win32 init networking, else just does nothin'
...
// BG: added possibility to select between ascii and HTML body
bool mailer::setmessage(const std::string& newmessage, int nMessageType /*= MAILER_BODYTYPE_TEXT*/) {
...
// BG: added possibility to select between ascii and HTML body
bool mailer::setmessage(const std::vector<char>& newmessage, int nMessageType /*= MAILER_BODYTYPE_TEXT*/) {
	if(!newmessage.size())
		return false;
...
void mailer::operator()() {
	...
	if(lookupMXRecord) {
		...
		hostent* host = 0;
		if(addr) {
			// BG: no need to get host name, IP is all we need
   ...
   const int buffsize(1024);
	...
	for(std::vector<SOCKADDR_IN>::const_iterator address = adds.begin();
      ...
      if(!Connect(s, *address, m_bTraceEnabled, m_szTraceFile)) {
		...
		int len1;
      if(!Recv(len1, s, buff, buffsize, 0, m_bTraceEnabled, m_szTraceFile)) {
         returnstring = "554 Transaction failed: server connect response error.";
			continue;
      }

		// get our hostname to pass to the smtp server
		// BG: use the name of the smtp server, rather than the host name, which is known on the local network only
		std::string hn = nameserver;
		int pos = hn.find("smtp.");
		if(pos == 0)
			hn = hn.replace(0, 5, "");
#if 0
		char hn[buffsize] = "";
		...
		if(!Send(len1, s, commandline.c_str(), commandline.length(), 0, m_bTraceEnabled, m_szTraceFile)) {
		...
		if(returnstring.substr(0,3) != OK) {
			...
			// send HELO instead.
			// BG: before sending the 'HELO' commmand, issue a reset (RSET), because some servers refuse to receive 'HELO' after 'EHLO'
			std::string commandline_temp(std::string("RSET") + std::string("\r\n"));
			if(!Send(len1, s, commandline_temp.c_str(), commandline_temp.length(), 0, m_bTraceEnabled, m_szTraceFile)) 
			{
            ...
            if(Send(len1, s, "QUIT\r\n", 6, 0, m_bTraceEnabled, m_szTraceFile)) {
               char dummy[buffsize];
               Recv(len1, s, dummy, buffsize, 0, m_bTraceEnabled, m_szTraceFile);
            }
				Closesocket(s, m_bTraceEnabled, m_szTraceFile);
      ...
      if(!Recv(len1, s, buff, buffsize, 0, m_bTraceEnabled, m_szTraceFile)) {
		...
		// Sending the email
		if(!Send(len1, s, smtpheader.c_str(), smtpheader.length(), 0, m_bTraceEnabled, m_szTraceFile)) {
      ...
      // for future reference the server is meant to give a 221 response to a quit.
		...
		Closesocket(s, m_bTraceEnabled, m_szTraceFile); // disconnect
...
std::string mailer::makesmtpmessage() const {
	...
	if(ptm) {
		...
		// BG: Use 4 digit format (2006, instead of 06), to avoid incorrect date format (RFC 2822)
		// std::ostringstream year;
		// year << ptm->tm_year;
		// str << year.str().substr(year.str().length() -2, 2) << " ";
		str << ptm->tm_year+1900 << " ";
		if(ptm->tm_hour < 10) // add a trailing zero if sigle digit
			str << "0";
		str << ptm->tm_hour << ":";
		if(ptm->tm_min < 10) // add a trailing zero if sigle digit
			str << "0";
		str << ptm->tm_min << ":";
		if(ptm->tm_sec < 10) // add a trailing zero if sigle digit
			str << "0";
		str << ptm->tm_sec << " ";
		
		// BG: Add timezone information (+-hhmm)
		tzset();
		int nTimeZone_Hours = (int)(-timezone/3600L);
		if(nTimeZone_Hours < 0)
		{
			str << "-";
			nTimeZone_Hours = -nTimeZone_Hours;
		}
		else
			str << "+";
		if(daylight)
			nTimeZone_Hours++;
		if(nTimeZone_Hours < 10) // add a trailing zero if sigle digit
			str << "0";
		str << nTimeZone_Hours << "00\r\n";
	...
	// BG: use MIME with boundaries if we have some attachments
	if(attachments.size()) {
		std::string boundary("bounds=_NextP_0056wi_0_8_ty789432_tp");
		...
		smtpheader += "--" + boundary + "\r\n";
// BG: added possibility to select between ascii and HTML body
		if(m_nMessageType == MAILER_BODYTYPE_HTML)
			smtpheader += "Content-type: text/html; charset=iso-8859-1\r\n";
		else
			smtpheader += "Content-type: text/plain; charset=iso-8859-1\r\n";
		smtpheader += "Content-transfer-encoding: 7BIT\r\n\r\n";
		// now add the plain text message first.
		...
		// BG: use MIME without boundaries if we have no attachments
		// use MIME 1.0
		smtpheader += "MIME-Version: 1.0\r\n";
...
bool mailer::gethostaddresses(std::vector<SOCKADDR_IN>& adds) {
   ...
   if(!Connect(s, addr, m_bTraceEnabled, m_szTraceFile)) {
   ...
   int ret;
   if(!Send(ret, s, (char*)dns, dnspos, 0, m_bTraceEnabled, m_szTraceFile)) {
      returnstring = "451 Requested action aborted: server seems to have disconnected.";
      return false;
   }
	if(Recv(ret, s, (char*)dns, 512, 0, m_bTraceEnabled, m_szTraceFile)) {
   	Closesocket(s, m_bTraceEnabled, m_szTraceFile);
...
bool mailer::authenticate(const std::string& servergreeting, const SOCKET& s) {
   ...
   if(type == LOGIN) {
      greeting = "auth " + at + "\r\n";
      if(!Send(len, s, greeting.c_str(), greeting.length(), 0, m_bTraceEnabled, m_szTraceFile)) {
         returnstring = "554 send failure: \"auth " + at + "\"";
         return false;
      }
      if(!Recv(len, s, buff, buffsize, 0, m_bTraceEnabled, m_szTraceFile)) {
