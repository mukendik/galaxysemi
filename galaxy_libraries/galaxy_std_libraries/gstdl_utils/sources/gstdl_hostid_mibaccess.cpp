...
MibExtLoad::MibExtLoad(const char* MibDllName)
{
	m_Init = NULL;
    m_InitEx = NULL;
	m_Query = NULL;
...
#define			NUM_VARBIND_LIST			7
	...
	// DEBUG MICROLINEAR
	GSystemInfo_Debug_FunctionEnter("GetNICCount");
...
// If the user wants to get the # of NICs in the system, then use only Num Entries.
	...
	// DEBUG MICROLINEAR
	GSystemInfo_Debug_WriteLongVar("Index", ifIndex);
	GSystemInfo_Debug_FunctionExit("GetNICCount", NICCount);
...
*/
UINT MibII::GetNICInfo(tSTRUCTNICINFO *pNICInfo)
{
	// DEBUG MICROLINEAR
	GSystemInfo_Debug_FunctionEnter("GetNICInfo");

	tSTRUCTNICINFO		tempStruct;
	UINT				i, j, validNICIndex = 0;

	// DEBUG MICROLINEAR
	char szDebugString[200];
	for(i = 0; i < m_ifCount; i++)
	{
		sprintf(szDebugString, "Interface %d: index=%lu, nb=%lu, MACL=%d", i, m_ifIndex[i], m_ifEntryNum[i], m_pNICInfo[i].MACLength);
		GSystemInfo_Debug_WriteString(szDebugString);
	}
	...
	for (i = 0; i < m_ifCount; i++)
	{
		...
		if(j < m_ifCount)
		{
			...
			int nMACLength = min(tempStruct.MACLength, sizeof(tempStruct.MAC));
			memcpy(tempStruct.MAC, m_pNICInfo[j].MAC, nMACLength);
	...
	// DEBUG MICROLINEAR
	GSystemInfo_Debug_FunctionExit("GetNICInfo", validNICIndex);

	return validNICIndex;
}
