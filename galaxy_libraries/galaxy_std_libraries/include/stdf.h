...
public:
    ...
    static int  GetCPUType(const std::string& lSTDFFilename);
    static int  GetStaticStdfVersion();

    //! \brief English days week names
    static char* sEnglishDayNames[];
    //! \brief Returns days of week name string from given index (from 0 to 6)
    //! \brief For the moment returns english days names but this function will be usefull to support other languages
    static char* GetDaysOfWeekString(const int &lDaysIndex);

    static char* sEnglishMonthNames[];
    //! \brief Get the name of the given month index from 0 (Jan) to 11 (Dec)
    static char* GetMonthName(const int &lMonthIndex);

    bool IsCompatible();

    // STDF File init.
    // Opens STDF file in STDF_READ or STDF_WRITE mode
    int	 Open(const char *szStdfFile,
              int iAccessMode,
              long lCacheSizeValue=-1,
              bool bValidityCheck=true);
    ...
    int	WriteRecord(LPStdfRecordReadInfo pclStdfRecordReadInfo, Stdf *pclStdfSource);
    ...
    int	DumpRecord(LPStdfRecordReadInfo pRecordReadInfo,Stdf *pStdfRecord,bool bFullRecord=false);

    // functions for site number deciphering
    DecipheringModes GetHeadAndSiteNumberDecipheringMode() const
    { return m_SiteNumberDecipheringMode; }

    void SetHeadAndSiteNumberDecipheringMode( DecipheringModes mode )
...
private:
    ...
    // decyphering mode for site number
    DecipheringModes m_SiteNumberDecipheringMode;

    static int sStdfVersion;    // contains the version calculated in the static function IsCompatible
};
