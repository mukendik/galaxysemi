...
#define MAILER_BODYTYPE_TEXT	0
#define MAILER_BODYTYPE_HTML	1
...
public:
	...
	mailer(const char* TOaddress, const char* FROMaddress,
		   ...
		   bool MXLookup = true, bool bTraceEnabled = false, const char* szTraceFile = NULL);
   ...
   mailer(bool MXLookup = false, unsigned short Port = SMTP_PORT, bool bTraceEnabled = false, const char* szTraceFile = NULL);
   ...
   void send();
	...
	// BG: added parameter to select between ascii and html body
	bool setmessage(const std::string& newmessage, int nMessageType = MAILER_BODYTYPE_TEXT);
	bool setmessage(const std::vector<char>& newmessage, int nMessageType = MAILER_BODYTYPE_TEXT);
...
private:
   ...
   // BG: type of message in the body: ascii, html
   int m_nMessageType;
   // BG: trace capability
   bool m_bTraceEnabled;
   const char *m_szTraceFile;
};
