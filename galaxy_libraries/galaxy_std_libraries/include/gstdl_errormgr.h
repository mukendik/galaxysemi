...
#define GDECLARE_END_ERROR_MAP(className)\
    ...
    mutable std::string     m_strLastErrorDesc##className;\
    ...
    std::string         GetLastErrorDescription##classname() const { return m_strLastErrorDesc##className; }
...
#define	GMAP_ERROR_EX(e,string,shortDesc)   case e:\
                                                {\
                                                    va_start(arglist, pclError);\
                                                    vsprintf(szBuffer,string,arglist);\
                                                    if(lpcszHeader)\
                                                    ...
                                                    strError += szBuffer;\
                                                    va_end(arglist);\
                                                    strErrorDescription = shortDesc;\
                                                }\
                                                break;

#define GEND_ERROR_MAP(className)		} /* switch */\
										m_clLastError##className += strError;\
                                        m_strLastErrorDesc##className = strErrorDescription;\
									}
...
#define GSET_ERROR6(className,errorCode,error_pointer,a1,a2,a3,a4,a5,a6)\
					XGFormatError##className(errorCode,NULL,error_pointer,a1,a2,a3,a4,a5,a6)
...
#define GSET_ERRORH6(className,errorCode,error_pointer,a1,a2,a3,a4,a5,a6,header)\
					XGFormatError##className(errorCode,header,error_pointer,a1,a2,a3,a4,a5,a6)
...
#define GGET_LASTERRORDESC(className,object_pointer)					((object_pointer)->m_strLastErrorDesc##className.c_str())
...
public:
    // ----------------------------------------------------------------------------------------------------------
    // Description: Retrieve the error message associated with this object.
    //				It catenates all strings from the internal list to return a single one.
    // Arguments:
    //			nMsgLevel:	If specified, the maximum number of catenates strings to put in the returned one.
    // Return: The string created from the catenate of string from internal list.
    // ----------------------------------------------------------------------------------------------------------
    ...
    //				It catenates all strings from the internal list to return a single one.
    // Arguments:
    //			strErrorMsg: error message should be stored into this string
    //			nMsgLevel:	If specified, the maximum number of catenates strings to put in the returned one.
    // Return: The string created from the catenate of string from internal list.
    // ----------------------------------------------------------------------------------------------------------
