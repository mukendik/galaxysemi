...
{
    #ifdef QT_DEBUG
        qDebug("ConnectionWidget::refresh");
    #endif
    tree->clear();
    ...
    for (int i = 0; i < connectionNames.count(); ++i)
    {
        ...
        if (db.isOpen())
        {
            ...
            for (int t = 0; t < tables.count(); ++t)
            {
                ...
                /*
                QSqlRecord r=db.record(tables.at(t));
                QStringList sl;
                for (int i=0; i<r.count(); i++)
                    sl.append(r.field(i).name()+ r.value(i).toString());
                table->setToolTip(0, sl.join(" "));
                */
                // ToDo : "show table status" to get the num of rows, the engine,...
            }
        }
    }
    if (!gotActiveDb)
...
{
    #ifdef QT_DEBUG
        qDebug("ConnectionWidget::setActive");
    #endif
    ...
    #ifdef QT_DEBUG
        qDebug("ConnectionWidget::setActive %s", activeDb.toLatin1().data());
    #endif
...
{
    #ifdef QT_DEBUG
        qDebug("ConnectionWidget::on_tree_itemActivated %s", item?item->text(0).toLatin1().data():"?");
    #endif
...
{
    #ifdef QT_DEBUG
        qDebug("ConnectionWidget::on_tree_currentItemChanged");
    #endif
