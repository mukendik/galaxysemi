...
{ 
    ...
    if(target) 
	{
		...
		*/
		...
		console.log('mouseDown on '+draggingElement.getAttribute('id') );
        if (draggingElement.getAttribute('fill-opacity')>0.5)
			draggingElement.setAttribute('fill-opacity', 0.2);
		else
			draggingElement.setAttribute('fill-opacity', 0.8);
...
function initialize() 
{
    var root=document.documentElement; //.getElementsBy("background");
	if (!root)
		console.log('root not found');

	//root.setAttribute("style","mask: url(#mask1)");
    ...
    for(var x = 0; x < 100; x=x+10) 
	{
		for(var y = 0; y < 100; y=y+10) 
		{
			...
			//shape.setAttribute("style","mask: url(#mask1)");
			//shape.setAttribute("stroke", "black");
			shape.setAttributeNS(DRAGNS, "enable", true);
			document.documentElement.appendChild(shape);
			/* var e=root.createSVGRect(); e.x=10; e.y=10; e.width=620; e.height=460;
