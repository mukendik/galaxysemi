...
union aword {
  ...
  UBYTE_08bits byte [4];
#ifdef __sparc__
  // Unix platform, keep definition as before.
  struct {
	unsigned int byte3:8;
	unsigned int byte2:8;
	unsigned int byte1:8;
...
#else
	// PC Platform (LITTLE_ENDIAN): swap definition order
...
// Operations
	...
	int			SetBufferFromFile(char *,char *);
	int			SetFileFromBuffer(char * szFileName,char *szMode);
	int			SetBufferFromBuffer(char *,int);
	void		SetBufferFromHexa(char *);
	void		GetBuffer(char **);
