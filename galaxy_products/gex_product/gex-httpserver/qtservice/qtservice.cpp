...
/*!
    ...
    \a name.
*/
QtServiceController::QtServiceController(const QString &name) : d_ptr(new QtServiceControllerPrivate())
{
	qDebug("QtServiceController::QtServiceController");
...
{
	qDebug("QtServiceController::install");
...
{
	qDebug("QtServiceController::start");
...
public slots:
    void slotStart()
    {
		qDebug("QtServiceStarter::slotStart");
        d_ptr->startService();
    }
private:
...
{
	qDebug("QtServiceBasePrivate::startService");
	q_ptr->start();
}

int QtServiceBasePrivate::run(bool asService, const QStringList &argList)
{
	qDebug("QtServiceBasePrivate::run");
...
{
	qDebug("QtServiceBase::QtServiceBase");
#if defined(QTSERVICE_DEBUG)
...
int QtServiceBase::exec()
{
	...
	if (d_ptr->args.size() > 1)
	{
		...
		else  if (a == QLatin1String("-h") || a == QLatin1String("-help"))
		{
			...
			qDebug(" unknown arg (%s).", a.toLatin1().data());
    }
	else
		qDebug(" no args.");
