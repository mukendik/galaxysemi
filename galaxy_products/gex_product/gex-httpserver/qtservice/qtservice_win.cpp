...
{
	qDebug("QtServiceBasePrivate::start");
    sysInit();
    if (!winServiceInit())
        return false;
	...
	qDebug("QtServiceBasePrivate::start: new HandlerThread...");
	...
	qDebug("QtServiceBasePrivate::start: tryAcquire...");
	if (!sys->startSemaphore.tryAcquire(1, 20000))
        return false;

	if (!ht->calledOk())
	{
		qDebug(" tryAcquire called not ok !");
        if (ht->runningAsConsole())
		{
			qDebug(" running as console...");
