...
//#define GexFtpQFtpPI_DEBUG
//#define GexFtpQFtpDTP_DEBUG
...
QBasicAtomicInt GexFtpQFtpCommand::idCounter = Q_BASIC_ATOMIC_INITIALIZER(1);

GexFtpQFtpCommand::GexFtpQFtpCommand(GexFtpQFtp::Command cmd, QStringList raw, const QByteArray &ba)
...
GexFtpQFtpCommand::GexFtpQFtpCommand(GexFtpQFtp::Command cmd, QStringList raw, QIODevice *dev)
...
GexFtpQFtpCommand::~GexFtpQFtpCommand()
{
    if (is_ba)
        delete data.ba;
}
...
/**********************************************************************
 *
 * GexFtpQFtpDTP implemenatation
 *
 *********************************************************************/
GexFtpQFtpDTP::GexFtpQFtpDTP(GexFtpQFtpPI *p, QObject *parent) :
    QObject(parent),
    socket(0),
    listener(this),
    pi(p),
...
{
    clearData();
    listener.setObjectName(QLatin1String("GexFtpQFtpDTP active state server"));
    connect(&listener, SIGNAL(newConnection()), SLOT(setupSocket()));
}

void GexFtpQFtpDTP::setData(QByteArray *ba)
{
    is_ba = true;
    data.ba = ba;
}

void GexFtpQFtpDTP::setDevice(QIODevice *dev)
{
    is_ba = false;
    data.dev = dev;
}

void GexFtpQFtpDTP::setBytesTotal(qint64 bytes)
{
    bytesTotal = bytes;
    bytesDone = 0;
...
void GexFtpQFtpDTP::connectToHost(const QString & host, quint16 port)
{
    ...
    socket->setObjectName(QLatin1String("GexFtpQFtpDTP Passive state socket"));
...
int GexFtpQFtpDTP::setupListener(const QHostAddress &address)
{
...
qint64 GexFtpQFtpDTP::read(char *data, qint64 maxlen)
{
    qint64 read;
    if (socket && socket->state() == QTcpSocket::ConnectedState) {
...
QByteArray GexFtpQFtpDTP::readAll()
{
    QByteArray tmp;
    if (socket && socket->state() == QTcpSocket::ConnectedState) {
        tmp = socket->readAll();
...
void GexFtpQFtpDTP::writeData()
{
    if (!socket)
        return;

    if (is_ba) {
#if defined(GexFtpQFtpDTP_DEBUG)
        qDebug("GexFtpQFtpDTP::writeData: write %d bytes", data.ba->size());
#endif
        if (data.ba->size() == 0)
...
#if defined(GexFtpQFtpDTP_DEBUG)
        qDebug("GexFtpQFtpDTP::writeData: write() of size %lli bytes", read);
#endif
        if (read > 0) {
...
void GexFtpQFtpDTP::dataReadyRead()
{
    writeData();
}

inline bool GexFtpQFtpDTP::hasError() const
{
    return !err.isNull();
}

inline QString GexFtpQFtpDTP::errorMessage() const
{
    return err;
}

inline void GexFtpQFtpDTP::clearError()
{
    err.clear();
}

void GexFtpQFtpDTP::abortConnection()
{
#if defined(GexFtpQFtpDTP_DEBUG)
    qDebug("GexFtpQFtpDTP::abortConnection, bytesAvailable == %lli",
...
static void _q_parseUnixDir(const QStringList &tokens, const QString &userName, QUrlInfo *info)
{
	...
	//    formats << QLatin1String("MMM dd  yyyy") << QLatin1String("MMM dd hh:mm") << QLatin1String("MMM  d  yyyy") // Qt version  
	//            << QLatin1String("MMM  d hh:mm") << QLatin1String("MMM  d yyyy") << QLatin1String("MMM dd yyyy");	// Qt version  
	
	formats << QLatin1String("MMM dd yyyy") << QLatin1String("MMM dd hh:mm") << QLatin1String("MMM d yyyy")			// Edited by Galaxy  
			<< QLatin1String("MMM d hh:mm") << QLatin1String("M d hh:mm") <<	QLatin1String("M d yyyy")			// Added by Galaxy  
			<< QLatin1String("MMM dd h:mm") << QLatin1String("MMM d h:mm") <<	QLatin1String("M d h:mm");			// Added by Galaxy  


    QString dateString = tokens.at(6);
	// remove all non-ascii characters from date string						// Added by Galaxy
	dateString = dateString.remove(QRegExp("[^\040-\177]")).simplified();	// Added by Galaxy
...
//    if (n == 2 || n == 4) {												// Qt version
	if (n == 2 || n == 4 || n == 5 || n == 7 || n == 8 || n == 9) {			// Added by Galaxy
        // Guess the year.
...
bool GexFtpQFtpDTP::parseDir(const QByteArray &buffer, const QString &userName, QUrlInfo *info)
{
    if (buffer.isEmpty())
        return false;
...
void GexFtpQFtpDTP::socketConnected()
{
    bytesDone = 0;
#if defined(GexFtpQFtpDTP_DEBUG)
    qDebug("GexFtpQFtpDTP::connectState(CsConnected)");
#endif
    emit connectState(GexFtpQFtpDTP::CsConnected);
}

void GexFtpQFtpDTP::socketReadyRead()
{
    if (!socket)
        return;
    ...
    if (pi->currentCommand().isEmpty()) {
        socket->close();
#if defined(GexFtpQFtpDTP_DEBUG)
        qDebug("GexFtpQFtpDTP::connectState(CsClosed)");
#endif
        emit connectState(GexFtpQFtpDTP::CsClosed);
        return;
    }

    if (pi->abortState == GexFtpQFtpPI::AbortStarted) {
        // discard data
        socket->readAll();
        return;
    }
    ...
    if (pi->currentCommand().startsWith(QLatin1String("LIST"))) {
...
#if defined(GexFtpQFtpDTP_DEBUG)
            qDebug("GexFtpQFtpDTP read (list): '%s'", line.constData());
#endif
        ...
        if (!is_ba && data.dev) {
            do {
                ...
                bytesDone += bytesRead;
#if defined(GexFtpQFtpDTP_DEBUG)
                qDebug("GexFtpQFtpDTP read: %lli bytes (total %lli bytes)", bytesRead, bytesDone);
#endif
...
#if defined(GexFtpQFtpDTP_DEBUG)
            qDebug("GexFtpQFtpDTP readyRead: %lli bytes available (total %lli bytes read)",
...
void GexFtpQFtpDTP::socketError(QAbstractSocket::SocketError e)
{
    if (e == QTcpSocket::HostNotFoundError) {
#if defined(GexFtpQFtpDTP_DEBUG)
        qDebug("GexFtpQFtpDTP::connectState(CsHostNotFound)");
#endif
        emit connectState(GexFtpQFtpDTP::CsHostNotFound);
    } else if (e == QTcpSocket::ConnectionRefusedError) {
#if defined(GexFtpQFtpDTP_DEBUG)
        qDebug("GexFtpQFtpDTP::connectState(CsConnectionRefused)");
#endif
        emit connectState(GexFtpQFtpDTP::CsConnectionRefused);
    }
}

void GexFtpQFtpDTP::socketConnectionClosed()
{
    if (!is_ba && data.dev) {
        clearData();
    }
...
void GexFtpQFtpDTP::socketBytesWritten(qint64 bytes)
{
    bytesDone += bytes;
#if defined(GexFtpQFtpDTP_DEBUG)
    qDebug("GexFtpQFtpDTP::bytesWritten(%lli)", bytesDone);
#endif
    emit dataTransferProgress(bytesDone, bytesTotal);
    if (callWriteData)
        writeData();
...
void GexFtpQFtpDTP::setupSocket()
{
    socket = listener.nextPendingConnection();
    socket->setObjectName(QLatin1String("GexFtpQFtpDTP Active state socket"));
    connect(socket, SIGNAL(connected()), SLOT(socketConnected()));
    connect(socket, SIGNAL(readyRead()), SLOT(socketReadyRead()));
    connect(socket, SIGNAL(error(QAbstractSocket::SocketError)), SLOT(socketError(QAbstractSocket::SocketError)));
    connect(socket, SIGNAL(disconnected()), SLOT(socketConnectionClosed()));
...
/**********************************************************************
 *
 * GexFtpQFtpPI implemenatation
 *
 *********************************************************************/
GexFtpQFtpPI::GexFtpQFtpPI(QObject *parent) :
    QObject(parent),
    rawCommand(false),
...
{
    commandSocket.setObjectName(QLatin1String("GexFtpQFtpPI_socket"));
...
void GexFtpQFtpPI::connectToHost(const QString &host, quint16 port)
{
    emit connectState(GexFtpQFtp::HostLookup);
...
*/
bool GexFtpQFtpPI::sendCommands(const QStringList &cmds)
{
    if (!pendingCommands.isEmpty())
        return false;

    if (commandSocket.state() != QTcpSocket::ConnectedState || state!=Idle) {
        emit error(GexFtpQFtp::NotConnected, GexFtpQFtp::tr("Not connected"));
...
void GexFtpQFtpPI::clearPendingCommands()
{
...
#if defined(GexFtpQFtpPI_DEBUG)
    qDebug("GexFtpQFtpPI send: ABOR");
#endif
...
void GexFtpQFtpPI::hostFound()
{
    emit connectState(GexFtpQFtp::Connecting);
}

void GexFtpQFtpPI::connected()
{
    state = Begin;
#if defined(GexFtpQFtpPI_DEBUG)
//    qDebug("GexFtpQFtpPI state: %d [connected()]", state);
#endif
    // try to improve performance by setting TCP_NODELAY
    commandSocket.setSocketOption(QAbstractSocket::LowDelayOption, 1);

    emit connectState(GexFtpQFtp::Connected);
}

void GexFtpQFtpPI::connectionClosed()
{
    commandSocket.close();
    emit connectState(GexFtpQFtp::Unconnected);
}

void GexFtpQFtpPI::delayedCloseFinished()
{
    emit connectState(GexFtpQFtp::Unconnected);
}

void GexFtpQFtpPI::error(QAbstractSocket::SocketError e)
{
    if (e == QTcpSocket::HostNotFoundError) {
        emit connectState(GexFtpQFtp::Unconnected);
        emit error(GexFtpQFtp::HostNotFound,
                    GexFtpQFtp::tr("Host %1 not found").arg(commandSocket.peerName()));
    } else if (e == QTcpSocket::ConnectionRefusedError) {
        emit connectState(GexFtpQFtp::Unconnected);
        emit error(GexFtpQFtp::ConnectionRefused,
                    GexFtpQFtp::tr("Connection refused to host %1").arg(commandSocket.peerName()));
    } else if (e == QTcpSocket::SocketTimeoutError) {
        emit connectState(GexFtpQFtp::Unconnected);
        emit error(GexFtpQFtp::ConnectionRefused,
                   GexFtpQFtp::tr("Connection timed out to host %1").arg(commandSocket.peerName()));
    }
}

void GexFtpQFtpPI::readyRead()
{
...
*/
bool GexFtpQFtpPI::processReply()
{
#if defined(GexFtpQFtpPI_DEBUG)
//    qDebug("GexFtpQFtpPI state: %d [processReply() begin]", state);
    if (replyText.length() < 400)
        qDebug("GexFtpQFtpPI recv: %d %s", 100*replyCode[0]+10*replyCode[1]+replyCode[2], replyText.toLatin1().constData());
    else
        qDebug("GexFtpQFtpPI recv: %d (text skipped)", 100*replyCode[0]+10*replyCode[1]+replyCode[2]);
#endif
    ...
    switch (state) {
        case Begin:
            if (replyCode[0] == 1) {
                ...
                state = Idle;
                emit finished(GexFtpQFtp::tr("Connected to host %1").arg(commandSocket.peerName()));
                break;
            }
...
#if defined(GexFtpQFtpPI_DEBUG)
//    qDebug("GexFtpQFtpPI state: %d [processReply() intermediate]", state);
#endif
    ...
    if (rawCommand) {
...
#if defined(GexFtpQFtpPI_DEBUG)
            qDebug("GexFtpQFtp: bad 227 response -- address and port information missing");
#endif
        ...
        if (portPos == -1) {
#if defined(GexFtpQFtpPI_DEBUG)
            qDebug("GexFtpQFtp: bad 229 response -- port information missing");
#endif
            // this error should be reported
        } else {
            ++portPos;
        ...
        emit connectState(GexFtpQFtp::LoggedIn);
    ...
    switch (state) {
        ...
        case Idle:
            if (dtp.hasError()) {
                emit error(GexFtpQFtp::UnknownError, dtp.errorMessage());
                dtp.clearError();
            }
            startNextCmd();
            break;
        ...
        case Failure:
            ...
            if (currentCmd.startsWith(QLatin1String("EPSV"))) {
                ...
                emit error(GexFtpQFtp::UnknownError, replyText);
            }
            if (state != Waiting) {
                state = Idle;
                startNextCmd();
...
#if defined(GexFtpQFtpPI_DEBUG)
//    qDebug("GexFtpQFtpPI state: %d [processReply() end]", state);
#endif
    return true;
}
...
*/
bool GexFtpQFtpPI::startNextCmd()
{
...
#if defined(GexFtpQFtpPI_DEBUG)
    if (state != Idle)
        qDebug("GexFtpQFtpPI startNextCmd: Internal error! GexFtpQFtpPI called in non-Idle state %d", state);
#endif
    if (pendingCommands.isEmpty()) {
        currentCmd.clear();
    ...
    pendingCommands.pop_front();
#if defined(GexFtpQFtpPI_DEBUG)
    qDebug("GexFtpQFtpPI send: %s", currentCmd.left(currentCmd.length()-2).toLatin1().constData());
#endif
    state = Waiting;
...
void GexFtpQFtpPI::dtpConnectState(int s)
{
    switch (s) {
        case GexFtpQFtpDTP::CsClosed:
            if (waitForDtpToClose) {
            ...
            readyRead();
            return;
        case GexFtpQFtpDTP::CsConnected:
            waitForDtpToConnect = false;
            startNextCmd();
            return;
        case GexFtpQFtpDTP::CsHostNotFound:
        case GexFtpQFtpDTP::CsConnectionRefused:
            emit error(GexFtpQFtp::ConnectionRefused,
                        GexFtpQFtp::tr("Connection refused for data connection"));
            startNextCmd();
            return;
        default:
            return;
...
int GexFtpQFtpPrivate::addCommand(GexFtpQFtpCommand *cmd)
{
    pending.append(cmd);
...
/**********************************************************************
 *
 * GexFtpQFtp implementation
 *
 *********************************************************************/
/*!
    \class GexFtpQFtp
    \brief The GexFtpQFtp class provides an implementation of the client side of FTP protocol.
    ...
    \snippet doc/src/snippets/code/src_network_access_GexFtpQFtp.cpp 0
    ...
    \snippet doc/src/snippets/code/src_network_access_GexFtpQFtp.cpp 1

    For this example the following sequence of signals is emitted
    (with small variations, depending on network traffic, etc.):

    \snippet doc/src/snippets/code/src_network_access_GexFtpQFtp.cpp 2
    ...
    \snippet doc/src/snippets/code/src_network_access_GexFtpQFtp.cpp 3

    You can then get details about the error with the error() and
    errorString() functions.

    For file transfer, GexFtpQFtp can use both active or passive mode, and
    it uses passive file transfer mode by default; see the
    documentation for setTransferMode() for more details about this.

    Call setProxy() to make GexFtpQFtp connect via an FTP proxy server.
    ...
    \warning The current version of GexFtpQFtp doesn't fully support
...
/*!
    Constructs a GexFtpQFtp object with the given \a parent.
*/
GexFtpQFtp::GexFtpQFtp(QObject *parent)
    : QObject(*new GexFtpQFtpPrivate, parent)
{
    Q_D(GexFtpQFtp);
...
*/
GexFtpQFtp::GexFtpQFtp(QObject *parent, const char *name)
    : QObject(*new GexFtpQFtpPrivate, parent)
{
    Q_D(GexFtpQFtp);
...
/*!
    \fn void GexFtpQFtp::stateChanged(int state)
...
/*!
    \fn void GexFtpQFtp::listInfo(const QUrlInfo &i);
...
/*!
    \fn void GexFtpQFtp::commandStarted(int id)
...
/*!
    \fn void GexFtpQFtp::commandFinished(int id, bool error)
...
/*!
    \fn void GexFtpQFtp::done(bool error)
...
/*!
    \fn void GexFtpQFtp::dataTransferProgress(qint64 done, qint64 total)
    ...
    is possible that the GexFtpQFtp class is not able to determine the total
...
/*!
    \fn void GexFtpQFtp::rawCommandReply(int replyCode, const QString &detail);
...
*/
int GexFtpQFtp::connectToHost(const QString &host, quint16 port)
{
    QStringList cmds;
    cmds << host;
    cmds << QString::number((uint)port);
    int id = d_func()->addCommand(new GexFtpQFtpCommand(ConnectToHost, cmds));
...
*/
int GexFtpQFtp::login(const QString &user, const QString &password)
{
    QStringList cmds;
    ...
    return d_func()->addCommand(new GexFtpQFtpCommand(Login, cmds));
}
...
/*!
    ...
    \sa stateChanged() commandStarted() commandFinished()
*/
int GexFtpQFtp::close()
{
    return d_func()->addCommand(new GexFtpQFtpCommand(Close, QStringList(QLatin1String("QUIT\r\n"))));
}

/*!
    Sets the current FTP transfer mode to \a mode. The default is GexFtpQFtp::Passive.

    \sa GexFtpQFtp::TransferMode
*/
int GexFtpQFtp::setTransferMode(TransferMode mode)
{
    int id = d_func()->addCommand(new GexFtpQFtpCommand(SetTransferMode, QStringList()));
    d_func()->pi.transferConnectionExtended = true;
...
/*!
    ...
    GexFtpQFtp does not support FTP-over-HTTP proxy servers. Use
    QNetworkAccessManager for this.
*/
int GexFtpQFtp::setProxy(const QString &host, quint16 port)
{
    QStringList args;
    args << host << QString::number(port);
    return d_func()->addCommand(new GexFtpQFtpCommand(SetProxy, args));
}
...
*/
int GexFtpQFtp::list(const QString &dir)
{
    QStringList cmds;
    ...
    return d_func()->addCommand(new GexFtpQFtpCommand(List, cmds));
}
...
/*!
    ...
    \sa commandStarted() commandFinished()
*/
int GexFtpQFtp::cd(const QString &dir)
{
    return d_func()->addCommand(new GexFtpQFtpCommand(Cd, QStringList(QLatin1String("CWD ") + dir + QLatin1String("\r\n"))));
}
...
/*!
    ...
    commandFinished()
*/
int GexFtpQFtp::get(const QString &file, QIODevice *dev, TransferType type)
{
    QStringList cmds;
    ...
    return d_func()->addCommand(new GexFtpQFtpCommand(Get, cmds, dev));
}
...
*/
int GexFtpQFtp::put(const QByteArray &data, const QString &file, TransferType type)
{
    QStringList cmds;
    if (type == Binary)
        cmds << QLatin1String("TYPE I\r\n");
    ...
    return d_func()->addCommand(new GexFtpQFtpCommand(Put, cmds, data));
}
...
/*!
    ...
    emitted).
*/
int GexFtpQFtp::put(QIODevice *dev, const QString &file, TransferType type)
{
    QStringList cmds;
    if (type == Binary)
        cmds << QLatin1String("TYPE I\r\n");
    ...
    cmds << QLatin1String("STOR ") + file + QLatin1String("\r\n");
    return d_func()->addCommand(new GexFtpQFtpCommand(Put, cmds, dev));
}
...
/*!
    ...
    \sa commandStarted() commandFinished()
*/
int GexFtpQFtp::remove(const QString &file)
{
    return d_func()->addCommand(new GexFtpQFtpCommand(Remove, QStringList(QLatin1String("DELE ") + file + QLatin1String("\r\n"))));
}
...
/*!
    ...
    \sa commandStarted() commandFinished()
*/
int GexFtpQFtp::mkdir(const QString &dir)
{
    return d_func()->addCommand(new GexFtpQFtpCommand(Mkdir, QStringList(QLatin1String("MKD ") + dir + QLatin1String("\r\n"))));
}
...
/*!
    ...
    \sa commandStarted() commandFinished()
*/
int GexFtpQFtp::rmdir(const QString &dir)
{
    return d_func()->addCommand(new GexFtpQFtpCommand(Rmdir, QStringList(QLatin1String("RMD ") + dir + QLatin1String("\r\n"))));
}
...
/*!
    ...
    \sa commandStarted() commandFinished()
*/
int GexFtpQFtp::rename(const QString &oldname, const QString &newname)
{
    QStringList cmds;
    cmds << QLatin1String("RNFR ") + oldname + QLatin1String("\r\n");
    cmds << QLatin1String("RNTO ") + newname + QLatin1String("\r\n");
    return d_func()->addCommand(new GexFtpQFtpCommand(Rename, cmds));
}
...
/*!
    ...
    perform has an equivalent GexFtpQFtp function, we recommend using the
...
*/
int GexFtpQFtp::rawCommand(const QString &command)
{
    QString cmd = command.trimmed() + QLatin1String("\r\n");
    return d_func()->addCommand(new GexFtpQFtpCommand(RawCommand, QStringList(cmd)));
}
...
*/
qint64 GexFtpQFtp::bytesAvailable() const
{
    return d_func()->pi.dtp.bytesAvailable();
}

/*! \fn qint64 GexFtpQFtp::readBlock(char *data, quint64 maxlen)
...
*/
qint64 GexFtpQFtp::read(char *data, qint64 maxlen)
{
...
/*!
    ...
    \sa currentCommand()
*/
int GexFtpQFtp::currentId() const
{
    if (d_func()->pending.isEmpty())
        return 0;
...
/*!
    ...
    \sa currentId()
*/
GexFtpQFtp::Command GexFtpQFtp::currentCommand() const
{
    if (d_func()->pending.isEmpty())
        return None;
...
/*!
    ...
    \sa get() put()
*/
QIODevice* GexFtpQFtp::currentDevice() const
{
    if (d_func()->pending.isEmpty())
        return 0;
    GexFtpQFtpCommand *c = d_func()->pending.first();
    if (c->is_ba)
        return 0;
    return c->data.dev;
}
...
*/
bool GexFtpQFtp::hasPendingCommands() const
{
...
*/
void GexFtpQFtp::clearPendingCommands()
{
...
*/
GexFtpQFtp::State GexFtpQFtp::state() const
{
    return d_func()->state;
}
...
*/
GexFtpQFtp::Error GexFtpQFtp::error() const
{
    return d_func()->error;
}
...
/*!
    ...
    tr().
*/
QString GexFtpQFtp::errorString() const
{
...
/*! \internal
*/
void GexFtpQFtpPrivate::_q_startNextCommand()
{
    Q_Q(GexFtpQFtp);
    if (pending.isEmpty())
        return;
    GexFtpQFtpCommand *c = pending.first();

    error = GexFtpQFtp::NoError;
    errorString = QT_TRANSLATE_NOOP(GexFtpQFtp, QLatin1String("Unknown error"));
    ...
    // through.
    if (c->command == GexFtpQFtp::Login && !proxyHost.isEmpty()) {
    ...
    if (c->command == GexFtpQFtp::SetTransferMode) {
        _q_piFinished(QLatin1String("Transfer mode set"));
    } else if (c->command == GexFtpQFtp::SetProxy) {
    ...
    } else if (c->command == GexFtpQFtp::ConnectToHost) {
        ...
        } else if (c->command == GexFtpQFtp::Get) {
            if (!c->is_ba && c->data.dev) {
                pi.dtp.setDevice(c->data.dev);
            }
        } else if (c->command == GexFtpQFtp::Close) {
            state = GexFtpQFtp::Closing;
...
/*! \internal
*/
void GexFtpQFtpPrivate::_q_piFinished(const QString&)
{
    if (pending.isEmpty())
        return;
    GexFtpQFtpCommand *c = pending.first();

    if (c->command == GexFtpQFtp::Close) {
        ...
        // signal.
        if (state != GexFtpQFtp::Unconnected) {
...
/*! \internal
*/
void GexFtpQFtpPrivate::_q_piError(int errorCode, const QString &text)
{
	Q_Q(GexFtpQFtp);
    ...
    // non-fatal errors
    if (c->command == GexFtpQFtp::Get && pi.currentCommand().startsWith(QLatin1String("SIZE "))) {
        pi.dtp.setBytesTotal(-1);
        return;
    } else if (c->command==GexFtpQFtp::Put && pi.currentCommand().startsWith(QLatin1String("ALLO "))) {
        return;
    }

    error = GexFtpQFtp::Error(errorCode);
    switch (q->currentCommand()) {
        case GexFtpQFtp::ConnectToHost:
            errorString = QString::fromLatin1(QT_TRANSLATE_NOOP("GexFtpQFtp", "Connecting to host failed:\n%1"))
                          .arg(text);
            break;
        case GexFtpQFtp::Login:
            errorString = QString::fromLatin1(QT_TRANSLATE_NOOP("GexFtpQFtp", "Login failed:\n%1"))
                          .arg(text);
            break;
        case GexFtpQFtp::List:
            errorString = QString::fromLatin1(QT_TRANSLATE_NOOP("GexFtpQFtp", "Listing directory failed:\n%1"))
                          .arg(text);
            break;
        case GexFtpQFtp::Cd:
            errorString = QString::fromLatin1(QT_TRANSLATE_NOOP("GexFtpQFtp", "Changing directory failed:\n%1"))
                          .arg(text);
            break;
        case GexFtpQFtp::Get:
            errorString = QString::fromLatin1(QT_TRANSLATE_NOOP("GexFtpQFtp", "Downloading file failed:\n%1"))
                          .arg(text);
            break;
        case GexFtpQFtp::Put:
            errorString = QString::fromLatin1(QT_TRANSLATE_NOOP("GexFtpQFtp", "Uploading file failed:\n%1"))
                          .arg(text);
            break;
        case GexFtpQFtp::Remove:
            errorString = QString::fromLatin1(QT_TRANSLATE_NOOP("GexFtpQFtp", "Removing file failed:\n%1"))
                          .arg(text);
            break;
        case GexFtpQFtp::Mkdir:
            errorString = QString::fromLatin1(QT_TRANSLATE_NOOP("GexFtpQFtp", "Creating directory failed:\n%1"))
                          .arg(text);
            break;
        case GexFtpQFtp::Rmdir:
            errorString = QString::fromLatin1(QT_TRANSLATE_NOOP("GexFtpQFtp", "Removing directory failed:\n%1"))
                          .arg(text);
            break;
        default:
            errorString = text;
...
/*! \internal
*/
void GexFtpQFtpPrivate::_q_piConnectState(int connectState)
{
    state = GexFtpQFtp::State(connectState);
    emit q_func()->stateChanged(state);
    if (close_waitForStateChange) {
        close_waitForStateChange = false;
        _q_piFinished(QLatin1String(QT_TRANSLATE_NOOP("GexFtpQFtp", "Connection closed")));
    }
}

/*! \internal
*/
void GexFtpQFtpPrivate::_q_piFtpReply(int code, const QString &text)
{
    if (q_func()->currentCommand() == GexFtpQFtp::RawCommand) {
        pi.rawCommand = true;
...
//#include "GexFtpQFtp.moc"

//#include "moc_GexFtpQFtp.cpp"
