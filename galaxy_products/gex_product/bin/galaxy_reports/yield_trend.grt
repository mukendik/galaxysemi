...
<galaxy_template version="0.7">
	...
	<report name="Yield trend report">
		...
		<datasets>
			<dataset ID="DS1" DatabaseName="vishay_dourdan" export="true" WithOverall="true">
			</dataset>
		</datasets>
	
		<title><![CDATA[Overall $P{P_TITLE} Yields]]></title>
		...
		<group dataset="DS1" Split="DS1:SPLIT" Aggregate="DS1:AGGREGATECHART" Serie="DS1:SERIE" Order="DS1:ORDER">
			<chart visible="$P{P_SHOWCHARTS}">
				...
				<yaxis visible="true" pos="left" name="Yield (%)" type="$P{P_YIELD_AXIS_STYLE}" min="$P{P_CHART_LEFTAXIS_MIN}" max="$P{P_CHART_LEFTAXIS_MAX}">
					<serie  name="$FSerie + ' - Yield'" value="$P{P_YIELD}" printwhen="($FSerie=='Overall') ? $P{P_OVERALL_YIELD} : true" /> 
				</yaxis>
				<yaxis visible="$P{P_SHOW_VOL_IN_CHART}" pos="right" name="Volume" type="$P{P_VOL_AXIS_STYLE}" >
					<serie  name="$FSerie + ' - Volume'" value="$P{P_VOLUME}" printwhen="($FSerie=='Overall') ? $P{P_OVERALL_VOLUME} : true" />
				</yaxis>
