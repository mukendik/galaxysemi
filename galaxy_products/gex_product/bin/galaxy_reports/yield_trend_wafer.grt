...
<galaxy_template version="0.7">
	...
	<report name="Yield trend report">
		...
		<group dataset="DS1" Split="DS1:SPLIT" Aggregate="DS1:AGGREGATE" Serie="DS1:SERIE" Order="DS1:ORDER">
			<chart visible="$P{P_SHOWCHARTS}">
				...
				<yaxis visible="$P{P_SHOW_VOL_IN_CHART}" pos="right" name="Wafer Volume" type="$P{P_VOL_AXIS_STYLE}" >
					<serie  name="$FSerie + ' - Wafer volume'" value="$P{P_VOLUME}" />
				</yaxis>
			...
			<table type="vertical" visible="$P{P_SHOWTABLES}">
				<header>
					<line>
						...
						<cell value="String('Wafer Volume')" align="center"/>
		...
		<summary><![CDATA[Notes :<br> - Total parts measures have been computed using consolidated data. <br> - The Yield measure is computed from the total number of good parts and the total $P{P_SUMMARY}.<br>]]></summary>
