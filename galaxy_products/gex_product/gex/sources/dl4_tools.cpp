...
BOOL DoubleToString (double d, char *s_ret, short max_len)
{
    ...
    if (!d)
  {
    ...
    BOOL iret = EngineeringNotation ( d, s, max_len, gex_min(max_len-8,12)  );
...
BOOL FloatToString (float d, char *s_ret, short max_len, short sig_digits /* = 0 */, BOOL fixed_decimal /* = false */)
{
    ...
    if (!d)
  {
    ...
    BOOL iret = EngineeringNotation ((double) d, s, max_len, gex_min(max_len-5,7)  );
...
//    char* dummy;
    ...
    if(input_variable < 0)
    {
      ...
      strncpy(output_var_str,
              s_tmp,gex_min(StrLen(s_tmp) + 1, max_output_str_len));
...
BOOL STRING::Overwrite (short i, const char *c, short width)
{
  ...
  if (org_len < i + width)
  {
    ...
    else
    {
      ...
      s[alloc_size-1] = 0;
      width = gex_min(width,idx_of_last_non_space+1);
    }
  }
...
GENERIC_STORED_OBJECT_WRAPPER::~GENERIC_STORED_OBJECT_WRAPPER (void)
{
  if (m_data_dymamically_allocated == true)
    delete [] (BYTE*) m_data;
}
