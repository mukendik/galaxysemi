...
const char msgFuncDiffers[]   = "function differs from previous forward declaration ";
const char msgFuncNotimp[]    = "function not implemented ";
const char msgVarDiffers[]    = "identifier differs from previous extern declaration ";
...
ZBoolean inStr(char ch, const char* s)
{
   ZFUNCTRACE_DEVELOP("inStr(char ch, const char* s)");
   while (*s && ch != *s) s++;
   return *s && ch == *s;
} // inStr
...
ZBoolean validIdBeginChar(char ch)
{
   ...
   if ('a' <= ch && ch <= 'z') return zTrue;
   if ('A' <= ch && ch <= 'Z') return zTrue;
   return inStr(ch, (char*)"$_");
} // validIdBeginChar

ZBoolean validIdChar(char ch)
{
   ZFUNCTRACE_DEVELOP("validIdChar(char ch)");
   if ('0' <= ch && ch <= '9') return zTrue;
...
ZString pureVarName(const ZString& aVarName)
{
   ...
   if (!pos) return ZString(aVarName);
...
void ZCsl::checkIdent(char* s)
{
   ...
   if (!validIdBeginChar(*s)) throwExcept(msgInvIdent);
   while (validIdChar(*s) || inStr(*s,(char*)"[]")) s++;
   if (*s) throwExcept(msgInvIdent);
} // checkIdent
...
void ZCsl::trace(char aFlag, const char* aMessage)
{
   ...
   if (aFlag == '-' && iTraceLevel > 0) iTraceLevel--;
   ZString space;
...
ZCsl::Function* ZCsl::findFunc(const ZString& name, ZBoolean fail)
{
   ...
   if (fail) throwExcept(msgFuncNotFound, name);
   return 0;
} // findFunc
...
ZCsl::Function* ZCsl::addFunc(Function* aFunc, ZBoolean aStatic)
{
   ...
   if (f) {
      if (f->iFunc || f->iPsize) throwExcept(msgFuncAlrdyDef, aFunc->iName);
      ...
      while (iPar && aPar) {
         if (iPar->iName       != aPar->iName    ||
             ...
             iPar->iIsOptional != aPar->iIsOptional) break;
         iPar = iPar->iNext;
         aPar = aPar->iNext;
      } // while
      ...
      if (f->iParams) delete f->iParams;
      f->iParams = aFunc->iParams;
      aFunc->iParams = 0;

      if (f->iInst) delete f->iInst;
      f->iInst = aFunc->iInst;
      aFunc->iInst = 0;

      if (f->iPcode) delete f->iPcode;
...
void ZCsl::exec(Function* aFunc)
{
   ...
   try {
      ...
      if (par) {
         ...
         while (par) {
            argc++;
            if (par->iIsOptional) anyOpts = zTrue;
            par = par->iNext;
         } // while
         if (anyOpts) {
            ZString argcStr = pop();
         ...
         for (int p = 0; p < argc; p++) {
            iCalls->iBlocks->addVar(par->iName, pop(), par->iIsConst, par->iIsRef);
            par = par->iNext;
         } // for
      } // if
...
void ZCsl::push(const ZString& aArg)
{
   ...
   if (iTos == iStackLimit) {
      ...
      for (int i = 0; i < iStackSize; i++) newStack[i] = iStack[i];
      delete [] iStack;
      iStack = newStack;
...
ZString ZCsl::pop()
{
   ...
   if (iTos == iStack) throwExcept(msgStackUdr);
   return *iTos--;
} // pop


void ZCsl::throwExcept(const char *aMsg, const ZString& aInfo)
{
    ZString msg(aMsg);
    throwExcept(msg,aInfo);
}
...
void ZCsl::throwExcept(const ZString& aMsg, const ZString& aInfo)
{
   ...
   switch (iState) {
      case isCompiling:
         if (iInput) {
            msg = iInput->iFileName+"(Ln "
                  +ZString(iInput->iSymLine)
                  +", Col "
                  +ZString(iInput->iSymPos)
                  +") : "
                  +aMsg;
         } // if
         break;
...
void ZCsl::showIdents(Block* b, const char* lead, long& depth)
{
   ZFUNCTRACE_DEVELOP("ZCsl::showIdents(Block* b, const char* lead, long& depth)");
   Variable* v = b->iVars;
   while (v && depth > 0) {
      ZString msg(lead);
      ...
      if (v->iIsExtern) {
         if (v->dims())
            for (int d = 0; d < v->dims(); d++) msg += "[]";
         msg += ", unallocated external";
      } else {
         if (v->dims()) {
            for (int d = 0; d < v->dims(); d++) msg += "["+ZString(v->iDims[d])+"]";
         } else {
            v->match(v->iName);
            msg += " = "+v->value();
         } // if
...
{
   ZFUNCTRACE_DEVELOP("ZCsl::showFuncs(Function* f, ZBoolean v, long& depth)");
   while (f && depth > 0) {
...
void ZCsl::showLibs(long& /*depth*/)
{
   ...
   // Phil jan-2002
   log("Library name...: Gex Library");
   log("Library name...: System Library");
   log("Library name...: String sysLibrary");
   log("Library name...: Math sysLibrary");
   log("Library name...: File sysLibrary");
} // showIdents
