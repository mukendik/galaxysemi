...
ZExport0 ZPointerlist::ZPointerlist(const ZPointerlist& aPointerlist) :
   iCount(0),
   iSize(0),
   iList(0)
{
   ZFUNCTRACE_DEVELOP("ZPointerlist::ZPointerlist(const ZPointerlist& aPointerist)");
   *this = aPointerlist;
} // ZPointerlist
...
ZExport(ZPointerlist&) ZPointerlist::drop()
{
   ...
   for (long i = 0; i < iCount; i++) del(iList[i]);
   iCount = 0;
   return *this;
} // ZPointerlist::drop
