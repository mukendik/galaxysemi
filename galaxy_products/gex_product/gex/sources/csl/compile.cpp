...
void ZCsl::getSym0()
{
   ...
   switch (iSym) {
      case symBegin:     cerr << "symBegin:   "; break;
      case symEnd:       cerr << "symEnd:     "; break;
      case symLparen:    cerr << "symLparen:  "; break;
      case symRparen:    cerr << "symRparen:  "; break;
      case symLbrack:    cerr << "symLbrack:  "; break;
      case symRbrack:    cerr << "symRbrack:  "; break;
      case symRet:       cerr << "symRet:     "; break;
      case symNot:       cerr << "symNot:     "; break;
      case symVar:       cerr << "symVar:     "; break;
      case symConst:     cerr << "symConst:   "; break;
      case symRef:       cerr << "symRef:     "; break;
      case symDo:        cerr << "symDo:      "; break;
      case symIf:        cerr << "symIf:      "; break;
      case symElse:      cerr << "symElse:    "; break;
      case symWhile:     cerr << "symWhile:   "; break;
      case symFor:       cerr << "symFor:     "; break;
      case symTimes:     cerr << "symTimes:   "; break;
      case symSlash:     cerr << "symSlash:   "; break;
      case symPlus:      cerr << "symPlus:    "; break;
      case symMinus:     cerr << "symMinus:   "; break;
      case symMulBec:    cerr << "symMulBec:  "; break;
      case symDivBec:    cerr << "symDivBec:  "; break;
      case symAddBec:    cerr << "symAddBec:  "; break;
      case symSubBec:    cerr << "symSubBec:  "; break;
      case symIncr:      cerr << "symIncr:    "; break;
      case symDecr:      cerr << "symDecr:    "; break;
      case symComma:     cerr << "symComma:   "; break;
      case symBecomes:   cerr << "symBecomes: "; break;
      case symAnd:       cerr << "symAnd:     "; break;
      case symOr:        cerr << "symOr:      "; break;
      case symEql:       cerr << "symEql:     "; break;
      case symNeq:       cerr << "symNeq:     "; break;
      case symGtr:       cerr << "symGtr:     "; break;
      case symGeq:       cerr << "symGeq:     "; break;
      case symLss:       cerr << "symLss:     "; break;
      case symLeq:       cerr << "symLeq:     "; break;
      case symIdent:     cerr << "symIdent:   " << '<' << iIdent << '>'; break;
      case symLit:       cerr << "symLit:     " << '<' << iIdent << '>'; break;
      case symValue:     cerr << "symValue:   " << '<' << iValue << '>'; break;
      case symSemi:      cerr << "symSemi:    "; break;
      case symThrow:     cerr << "symThrow:   "; break;
      case symTry:       cerr << "symTry:     "; break;
      case symCatch:     cerr << "symCatch:   "; break;
      case symStatic:    cerr << "symStatic:  "; break;
      case symExtern:    cerr << "symExtern:  "; break;
      case symSizeof:    cerr << "symSizeof:  "; break;
      case symResize:    cerr << "symResize:  "; break;
      case symExist:     cerr << "symExist:   "; break;
      case symBreak:     cerr << "symBreak:   "; break;
      case symEof:       cerr << "symEof:     "; break;
      case symContin:    cerr << "symContin:  "; break;
      case symDiv:       cerr << "symDiv:     "; break;
      case symMod:       cerr << "symMod:     "; break;
      case symDivIBec:   cerr << "symDivIBec: "; break;
      case symModBec:    cerr << "symModBec:  "; break;
      case symCat:       cerr << "symCat:     "; break;
      case symCatBec:    cerr << "symCatBec:  "; break;
      case symDir:       cerr << "symDir:     "; break;
      case symSwitch:    cerr << "symSwitch:  "; break;
      case symCase:      cerr << "symCase:    "; break;
      case symColon:     cerr << "symColon:   "; break;
      case symDefault:   cerr << "symDefault: "; break;
      case symTrace:     cerr << "symTrace:   "; break;
      default:;
   } // switch
   cerr << endl;
#endif
   if (iSym == symEof) {
      if (iInput->iIfStack) throwExcept(msgEndifNotFound);
...
ZBoolean ZCsl::skipCode(ZBoolean elseOk)
{
   ...
   for (ZBoolean endFound = zFalse; !endFound;) {
      while (iSym != symDir) {
         getSym0();
         if (iSym == symEof) throwExcept(msgEndifNotFound);
      } // while
      getSym0();
      switch (iSym) {
         case symIdent:
         ...
         case symIf:
            getSym0();
            if (skipCode(zTrue)) skipCode(zFalse);
            break;
         case symElse:
            getSym0();
            if (!elseOk) throwExcept(msgCloseIfUnexp);
            return zTrue;
         default:
...
ZCsl::IfRecord::IfRecord(ZBoolean aElse, IfRecord* aPrev) :
   ...
   iPrev(aPrev)
{
   ZFUNCTRACE_DEVELOP("ZCsl::IfRecord::IfRecord(ZBoolean aElse, IfRecord* aPrev)");
} // IfRecord
...
void ZCsl::closeIfDir()
{
   ...
   if (rec == 0) throwExcept(msgCloseIfUnexp);
...
void ZCsl::directive()
{
   ...
   if (dirsym != symIdent && dirsym!=symIf && dirsym!=symElse)
      throwExcept(msgDirExpct);
   ZString direct;
   if (dirsym == symIdent) direct = iIdent;
   if (dirsym == symElse) {
      if (iFlags & disIf) throwExcept(msgUnknownDir, direct);
   ...
   if (direct == "endif") {
      ...
      if (blast->iCode != cdPush) throwExcept(msgInitNonConst);
      if (dirsym == symIf) {
         if (iFlags & disIf) goto reject;
         if (blast->iText.asDouble()==0) {
            if (skipCode(zTrue))
               iInput->iIfStack = new IfRecord(zTrue, iInput->iIfStack);
         } else
            iInput->iIfStack = new IfRecord(zFalse, iInput->iIfStack);
      } else
      if (direct == "include") {
         if (iFlags & disInclude) goto reject;
         ZString fname =
            ZFile::locateFile(
               ZFile::addExtension(blast->iText, ".csl"),
               "CSLPATH"
            );
         ...
         if (fname.size()==0)
            fname =
               ZFile::locateFile(
                  ZFile::addExtension(blast->iText, ".hss"),
                  "CSSPATH"
               );
#endif
         if (fname.size()==0)
      ...
      if (direct == "loadScript") {
         if (iFlags & disLoadScript) goto reject;
         iInDirective = zFalse;
         loadScript(blast->iText);
      } else
      if (direct == "loadLibrary") {
         if (iFlags & disLoadLibrary) goto reject;
         loadLibrary(blast->iText);
      } else
      if (direct == "logFile") {
         if (iFlags & disLogFile) goto reject;
         if (blast->iText.size())
      ...
      if (direct == "list") {
         if (iFlags & disList) goto reject;
         iList = blast->iText.asDouble() ? zTrue : zFalse;
      } else
      if (direct == "error") {
         if (iFlags & disError) goto reject;
      ...
      if (direct == "message") {
         if (iFlags & disMessage) goto reject;
         cout << blast->iText << endl;
      } else {
         reject:
...
void ZCsl::getSym()
{
   ...
   getSym0();
   for (;;) {
      if (iInDirective || iSym != symDir) break;
      iInDirective = zTrue;
      try {
         directive();
      } // try
   ...
   ZString& ident)
{
   ...
   if (iSym == symLbrack) {
      ...
      while (iSym == symLbrack) {
         ...
         if (iSym != symRbrack) throwExcept(msgMissRbrack);
         getSym();
         if (iSym == symLbrack) txt += "][";
      } // while
      txt += "]";
      bInst->add(cdPush, txt);
      if (!cnst) bInst->add(cdAdd);
   } else
   ...
   ZString isNulString="";
	...
	if (iSym == symLparen) {
      ...
      if (iSym != symRparen) {
         Function* args = 0;
         for (;;) {
            if (!par) throwExcept(msgInvParCnt, func->iName);
            args = new Function(this, isNulString, args);
            if (par->iIsRef) {
               // by reference
               if (iSym != symIdent) throwExcept(msgIdentExpct, func->iName+" #"+ZString(argc+1));
               ...
               if (aFunc)
                  ...
                  if (!var) var = iBlock->findVar(varName);
               } // if
               if (var->iIsConst && !par->iIsConst)
                  throwExcept(msgConstDenied, func->iName+"<--"+varName);
            } else
               orOp(aFunc, hInst, args->iInst, tInst); // by value
            argc++;
            par = par->iNext;
            if (iSym != symComma) break;
            getSym();
         } // for
         Function* arg = args;
      ...
      if (par && !par->iIsOptional) throwExcept(msgInvParCnt, func->iName);
      if (iSym != symRparen) throwExcept(msgMissRparen);
      getSym();
      par = func->iParams;
      while (par) {
         if (par->iIsOptional) {
   ...
   if (iSym == symBegin) {
      ...
      if (iSym != symEnd) {
         for (;;index++) {
            ...
            if (iSym == symBegin)
               if (dims)
                  arrayInit(aFunc, hInst, bInst, tInst, name, dims+1);
               else
               ...
               if (!dims) {
                  ...
                  if (!var) var = iBlock->findVar(name);
         ...
         if (dims && index > *dims) *dims = index+1;
      } // if
      if (iSym != symEnd) throwExcept(msgEndNotFound);
      getSym();
   } // if
} // arrayInit
...
void ZCsl::getParamList(Function* aFunc, ZBoolean aComp)
{
   ...
   if (iSym != symLparen) throwExcept(msgMissLparen);
   getSym();
   ...
   while (iSym != symRparen) {
      if (iSym == symLbrack) {
         if (isOptional) throwExcept(msgUnexpSymbol, "[");
         isOptional = zTrue;
         getSym();
      } // if
      ...
      if (iSym == symConst)
         ...
         if (iSym != symVar) throwExcept(msgVarSymExpct);
      getSym();
      if (iSym == symRef) {
         isRef = zTrue;
         getSym();
      ...
      while (iSym == symLbrack && dim < MAXDIMS) {
         if (!isRef) throwExcept(msgArrParamRef);
         getSym();
         if (iSym != symRbrack) throwExcept(msgMissRbrack);
         parName += "[]";
         parName2 += "[1]";
      ...
      if (aComp) aFunc->iBlocks->addVar(parName2, isNulString, isConst);
      if (iSym != symComma) break;
      getSym();
   } // while
   if (isOptional) {
      if (iSym != symRbrack) throwExcept(msgMissRbrack);
      if (aComp) aFunc->iBlocks->addVar("argCount", isOne, zTrue);
      getSym();
   } // if
   if (aComp) {
      aFunc->iBlocks->addVar("cslFunction", isOne, zTrue);
#ifdef ZC_CSS_COMPATIBLE
      aFunc->iBlocks->addVar("cssFunction", isOne, zTrue);
#endif
   } // if
   if (iSym != symRparen) throwExcept(msgMissRparen);
   getSym();
   ...
   Instruction* tInst)
{
   ...
   if (iSym==symSizeof) {
      ...
      if (aFunc)
         ...
         if (!var) var = iBlock->findVar(ident);
      } // if
   ...
   if (iSym==symExist) {
      ...
      if (aFunc)
         ...
         var = iStat->findVar(ident, zFalse);
         if (!var) var = iBlock->findVar(ident, zFalse);
      } // if
      if (iInDirective)
   ...
   if (iSym==symIncr || iSym==symDecr) {
      ...
      index(aFunc, hInst, &varInst, tInst, ident);
      Instruction* vlast = varInst.last();
      if (vlast->iCode==cdPush) ident = vlast->iText;
      Variable* var;
      if (aFunc)
         var = aFunc->findVar(ident);
      else {
      ...
      if (var->iIsConst) throwExcept(msgWriteConst);
   ...
   if (iSym==symPlus || iSym==symMinus || iSym==symNot) {
      ...
      if (blast->iCode==cdPush) {
         switch (op) {
            case symMinus: {
               if (!isNumber(blast->iText.constBuffer())) throwExcept(msgInvOpStrings);
   ...
   if (iSym==symIdent || iSym==symValue || iSym==symLit || iSym==symLparen) {
      switch (iSym) {
         case symIdent: {
            ZString ident(iIdent);
            getSym();
            if (iSym == symLparen)
               ...
               if (iSym==symIncr || iSym==symDecr) {
                  if (var->iIsConst) throwExcept(msgWriteConst);
                  tInst->copy(&varInst)->add(iSym==symIncr ? cdIncV : cdDecV);
                  getSym();
               } // if
               if (vlast->iCode==cdPush && var->iIsKnown)
         ...
         case symLit: {
            ZString lit;
            do { lit += iIdent; getSym(); } while (iSym == symLit);
   ...
   Instruction* tInst)
{
   ...
   factor(aFunc, hInst, bInst, tInst);
   while (iSym==symTimes || iSym==symSlash || iSym==symDiv || iSym==symMod) {
      Symbol mul = iSym;
      getSym();
      Instruction f2;
      ...
      if (blast->iCode==cdPush && flast->iCode==cdPush) {
         ...
         if (!isNumber(t1.constBuffer()) || !isNumber(t2.constBuffer()))
   ...
   while (iSym==symPlus || iSym==symMinus || iSym==symCat) {
      ...
      if (blast->iCode==cdPush && flast->iCode==cdPush) {
         ...
         if (!isNumber(t1.constBuffer())
             ...
             || add==symCat) {
            if (add == symMinus) throwExcept(msgInvOpStrings);
            blast->iText += t2;
         } else {
   ...
   ZBoolean aSave)
{
   ...
   if (iSym==symBecomes || iSym==symCatBec ||
      ...
      if (l->iCode != cdLoad) throwExcept(msgMissLvalue);
      delete l->iNext;
      l->iCode = cdNop;
      l->iNext = 0;
      ...
      if (aSave && !cnst) bInst->add(cdDupl);
      bInst->add(&body);
      switch (symBec) {
         case symAddBec : bInst->add(cdAddV); break;
         case symSubBec : bInst->add(cdSubV); break;
         case symCatBec : bInst->add(cdCatV); break;
         case symMulBec : bInst->add(cdMulV); break;
         case symDivBec : bInst->add(cdDivV); break;
         case symDivIBec: bInst->add(cdDivIV); break;
         case symModBec : bInst->add(cdModV); break;
         default        : bInst->add(cdStore);
      } // switch
      if (aSave && cnst) bInst->add(cdPush,l->iText);
   } else {
      if (aSave)
         bInst->add(&body);
      else {
...
{
   ZFUNCTRACE_DEVELOP("ZCsl::expr(Function* aFunc, Instruction* bInst, ZBoolean aSave)");
      ...
      do {
         ...
         if (iSym==symBecomes) {
            ...
            if (iSym == symBegin) {
               long dims[MAXDIMS];
               int d;
               for (d = 0; d < MAXDIMS; d++) dims[d] = 0;
               Instruction body;
               arrayInit(aFunc, bInst, &body, &tail, rawName, dims);
               for (d = 0; d < MAXDIMS; d++) {
                  if (!dims[d]) break;
               ...
               bInst->add(cdPush)
                    ->add(cdPush,rawName)
                    ->add(varSym==symConst ? cdAllC : cdAllV)
                    ->add(&body);
               aFunc->iBlocks->addVar(rawName, isNulString, varSym==symConst);
            } else {
               expr(aFunc, bInst);
               Instruction* blast = bInst->last();
               bInst->add(&varadr)
                    ->add(varSym==symConst ? cdAllC : cdAllV);
               ZString val;
               if (blast->iCode==cdPush) val = blast->iText;
               aFunc->iBlocks->addVar(
                  vlast->iCode==cdPush ? vlast->iText : ident,
                  val,
                  varSym==symConst,
                  zFalse,
                  vlast->iCode==cdPush && blast->iCode==cdPush);
            } // if
            bInst->add(&tail);
         } else {
            if (varSym==symConst) throwExcept(msgInitExpct);
            aFunc->iBlocks->addVar(
               vlast->iCode==cdPush ? vlast->iText : ident);
            bInst->add(cdPush)->add(&varadr)->add(cdAllV)->add(&tail);
         } // if
      } while (iSym == symComma);
   } else
      while (zTrue) {
   ...
   int cntLevel)
{
   ...
   switch (iSym) {
      case symBreak: {
         if (!(flags & 1)) throwExcept(msgInvBreak);
         getSym();
         while (brkLevel--) bInst->add(cdClose);
         bInst->add(cdJmp, breakAddr);
         if (iSym != symSemi) throwExcept(msgSemiNotFound);
         getSym();
         break;
      }
      case symContin: {
         if (!(flags & 2)) throwExcept(msgInvContin);
         getSym();
         while (cntLevel--) bInst->add(cdClose);
      ...
      case symTry: {
         getSym();
         if (iSym != symBegin) throwExcept(msgBeginNotFound);
         aFunc->openBlock();
         bInst->add(cdOpen);
         ...
         if (iSym != symCatch) throwExcept(msgCatchNotFound);
         getSym();
         if (iSym != symLparen) throwExcept(msgMissLparen);
         getSym();
         if (iSym!=symVar && iSym!=symConst) throwExcept(msgVarSymExpct);
         Symbol varSym(iSym);
         getSym();
         if (iSym!=symIdent) throwExcept(msgIdentExpct);
         ZString ident(iIdent);
         getSym();
         if (iSym != symLbrack) throwExcept(msgMissLbrack);
         getSym();
         if (iSym != symRbrack) throwExcept(msgMissRbrack);
         getSym();
         if (iSym != symRparen) throwExcept(msgMissRparen);
         ...
         aFunc->iBlocks->addVar(ident+"[1]", isNulString, varSym==symConst);
      ...
      case symSwitch: {
         ...
         while (iSym == symCase) {
            ...
            // get expression
            expr(aFunc, bInst);
            if (iSym != symColon) throwExcept(msgColonNotFound);
            getSym();
            // compare
            bInst->add(cdEql);
            Instruction* ifInst = bInst->add(cdJF);
      ...
      case symDo: {
         ...
         if (iSym != symWhile) throwExcept(msgMissWhile);
         getSym();
         if (iSym != symLparen) throwExcept(msgMissLparen);
         getSym();
         Instruction cond;
      ...
      case symFor: {
         ...
         if (iSym != symSemi) simple(aFunc, bInst); // initializer
         if (iSym != symSemi) throwExcept(msgSemiNotFound);
         getSym();
         Instruction cond;
         if (iSym == symSemi)
            cond.add(cdPush,isOne);
         else
            expr(aFunc, &cond);
         Instruction* clast = cond.last();
         if (iSym != symSemi) throwExcept(msgSemiNotFound);
         getSym();
         Instruction trail;
         if (iSym != symRparen) simple(aFunc, &trail);
         if (iSym != symRparen) throwExcept(msgMissRparen);
         getSym();
         Instruction* condInst = bInst->next();
         ...
         if (clast->iCode==cdPush) {
            ...
            trailAddr = bInst->next();
            bInst->add(&trail);
            if (trailAddr == bInst->next()) trailAddr = condInst;
            bInst->add(cdJmp,condInst);
         ...
         while (condInst) {
            if (condInst->iCode == cdJmp) {
               if (condInst->iAddr == breakAddr)
                  ...
                  if (condInst->iAddr == continueAddr)
...
void ZCsl::packCode(Function* aFunc)
{
      ...
      switch (i->iCode) {
         ...
         case cdOpen:
            if (i->iText.length()) {
               ...
               Instruction* ii;
               for (ii = aFunc->iInst; i->iText != ii->iText; ii = ii->iNext) ;
               if (ii == i) aFunc->iTsize += i->iText.length()+1;
            } // if
         default:;
      } // switch
      i = i->iNext;
   ...
   if (aFunc->iPsize) {
      PackedInstruction* p = aFunc->iPcode = new PackedInstruction[aFunc->iPsize];
      if (aFunc->iTsize) aFunc->iText = new char[aFunc->iTsize];
      char* t = aFunc->iText;
...
ZExport(ZCsl&) ZCsl::loadScript(const ZString& aFileName, istream* aStr)
{
   ...
   ZFUNCTRACE_DEVELOP("ZCsl::loadScript(const ZString& aFileName, istream* aStr)");
   GSLOG(SYSLOG_SEV_INFORMATIONAL, QString("ZCsl::loadScript(const ZString& (%1), istream* aStr)")
   ...
   try {
      ...
      while (iSym==symVar    || iSym==symConst  ||
         ...
         Symbol varSym(iSym);
         if (iSym == symVar || iSym == symConst) getSym();
         if (iSym != symIdent) throwExcept(msgIdentExpct);
         ZString ident(iIdent);
         getSym();
         if (iSym == symLparen) {
            if (iExtern) throwExcept(msgSemiNotFound);
            if (varSym == symConst) throwExcept(msgInitExpct);
            ...
            if (iSym == symSemi) {
               ...
               getSym();
               while (iSym != symEnd) stmt(aFunc, bInst, 0, 0, 0);
               getSym();
               ZBoolean addRet(zFalse);
               if (!bInst->iNext)
                  addRet = zTrue;
               else
                  if (bInst->last()->iCode != cdRet) addRet = zTrue;
               if (addRet)
            ...
            if (varSym == symIdent) throwExcept(msgVarSymExpct);
            for (;;) {
               Instruction varInst;
               Instruction valInst;
               if (iExtern) {
                  ...
                  iBlock->addVar(ident, isNulString, varSym==symConst, zFalse, zFalse, zTrue);
               } else {
                  ZString rawName(ident);
                  ...
                  if (vlast->iCode!=cdPush) throwExcept(msgIndexNonConst);
                  if (iSym != symBecomes) {
                     if (varSym == symConst) throwExcept(msgInitExpct);
                     valInst.add(cdPush);
                  } else {
                  ...
                  if (vlast->iCode!=cdPush) throwExcept(msgInitNonConst);
                  Variable* var = iStat->findVar(ident, zFalse);
                  if (var) {
                     if (!var->iIsExtern) throwExcept(msgVarAlrdyDef);
                     if (!iStatic) throwExcept(msgVarDiffers, ident);
                  } else {
                  ...
                  if (arrInit) {
                     Instruction head, tail;
                     arrayInit(0, &head, &valInst, &tail, rawName, 0);
                  } // if
               } // if
               if (iSym != symComma) break;
               getSym();
