   ...
   iParent(aParent)
{
   ...
   if (aIsRef) {
      ...
      int dim;
      for (dim = 0; dim < MAXDIMS; dim++) iDims[dim] = 0;
      ...
      while (*a == '[' && dim < MAXDIMS) {
         a++;
         while ('0'<=*a && *a<='9') { iDims[dim] = 10 * iDims[dim] + *a - '0'; a++; }
         if (*a++ != ']' || !iDims[dim++])
            iParent->throwExcept(msgInvalidIndex, aVarName);
      } // while
      if (*a) iParent->throwExcept(msgInvalidIndex, aVarName);
      long sz = size();
      iVals = new ZString[sz];
      for (int i = 0; i < sz; i++) iVals[i] = aInitValue;
   } // if
} // Variable
...
ZString ZCsl::Variable::value() const
{
   ...
   if (iIsExtern) iParent->throwExcept(msgUnallocExtern, iName);
   if (iIndex < 0 || iIndex >= size()) iParent->throwExcept(msgInvalidIndex, iName);
   return iVals[iIndex];
} // value
...
int ZCsl::Variable::dims() const
{
   ...
   int i;
   for (i = 0; i < MAXDIMS; i++) if (!iDims[i]) break;
   return i;
} // dims

ZCsl::Variable& ZCsl::Variable::set(const ZString& aValue, ZBoolean aInitConst)
{
   ZFUNCTRACE_DEVELOP("ZCsl::Variable::set(const ZString& aValue, ZBoolean aInitConst)");
   if (iIsExtern) iParent->throwExcept(msgUnallocExtern, iName);
...
ZCsl::Variable& ZCsl::Variable::resize(const ZString& aVarName)
{
   ...
   int dim;
   for (dim = 0; dim < MAXDIMS; dim++) newDims[dim] = 0;
   dim = 0;
   while (*a == '[' && dim < MAXDIMS) {
      a++;
      while ('0'<=*a && *a<='9') { newDims[dim] = 10 * newDims[dim] + *a - '0'; a++; }
