...
*/
int CFont_3D::ReadFont(const char *font_name, struct glf_font *glff)
{
  ...
  if (fread(glff->font_name, 96, 1, fontf) != 1) {
    return GLF_ERROR;
  }
	glff->font_name[96] = 0;

  // Read total symbols in font
  if (fread(&glff->sym_total, 1, 1, fontf) != 1) {
    return GLF_ERROR;
  }
  ...
  for (i = 0; i < 28; i++) {
    if (fread(&temp, 1, 1, fontf) != 1) {  // Read unused data
      return GLF_ERROR;
    }
  }
	...
	{
    if (fread(&code,  1, 1, fontf) != 1) { return GLF_ERROR; } // symbol code
    if (fread(&verts, 1, 1, fontf) != 1) { return GLF_ERROR; } // vertexs count
    if (fread(&fcets, 1, 1, fontf) != 1) { return GLF_ERROR; } // facets count
    if (fread(&lns,   1, 1, fontf) != 1) { return GLF_ERROR; } // lines count
		...
		for (j=0; j<verts; j++)
		{
      if (fread(&tempfx, 4, 1, fontf) != 1) { return GLF_ERROR; }
      if (fread(&tempfy, 4, 1, fontf) != 1) { return GLF_ERROR; }
		...
		for (j=0; j<fcets; j++)
      if (fread(&glff->symbols[(int)code]->fdata[j*3], 3, 1, fontf) != 1) {
        return GLF_ERROR;
      }
		for (j=0; j<lns; j++)
      if (fread(&glff->symbols[(int)code]->ldata[j], 1, 1, fontf) != 1) {
        return GLF_ERROR;
      }
	}
	fclose(fontf);
...
*/
int CFont_3D::glfLoadFont(const char *font_name)
{
	int i;
...
int CFont_3D::glfUnloadFontD(int font_descriptor)
{
	int temp;
...
static void glfDrawWiredSymbol(char s)
{
	int i, cur_line;
...
void CFont_3D::glfDrawWiredSymbolF(int font_descriptor, char s)
{
	int temp;
...
void CFont_3D::DrawString(const char *s, void (*funct) (char s))
{
	int i;
	float sda, sdb;
	float distance=0.0F;
		...
		for (i=0; i<(int)strlen(s); i++)
		{
			if ((fonts[curfont]->symbols[(int)(s[i])] == NULL) || (s[i] == ' ')) {
				...
				if (s[i+1] == ' ') {
					...
					if (fonts[curfont]->symbols[(int)(s[i+1])] == NULL) {
						continue;
					}
...
void CFont_3D::glfDrawWiredString(const char *s)
{
	DrawString(s, &glfDrawWiredSymbol);
}

/* Draw wired string by font_descriptor */
void CFont_3D::glfDrawWiredStringF(int font_descriptor, const char *s)
{
	int temp;
...
static void glfDrawSolidSymbol(char s)
{
...
void CFont_3D::glfDrawSolidSymbolF(int font_descriptor, char s)
{
	int temp;
...
void CFont_3D::glfDrawSolidString(const char *s)
{
	DrawString(s, &glfDrawSolidSymbol);
}

/* Draw solid string by font_descriptor */
void CFont_3D::glfDrawSolidStringF(int font_descriptor, const char *s)
{
	int temp;
...
static void glfDraw3DWiredSymbol(char s)
{
	int i, cur_line;
...
void CFont_3D::glfDraw3DWiredSymbolF(int font_descriptor, char s)
{
	int temp;
...
void CFont_3D::glfDraw3DWiredString(const char *s)
{
	DrawString(s, &glfDraw3DWiredSymbol);
}

/* Draw 3D wired string by font_descriptor */
void CFont_3D::glfDraw3DWiredStringF(int font_descriptor, const char *s)
{
	int temp;
...
static void glfDraw3DSolidSymbol(char s)
{
	int i, j, cur_line, flag;
	float x, y, bx=0.0F, by=0.0F;
	unsigned char *b; /* Face pointer   */
	float *vp;        /* Vertex pointer */
	float *tvp;       /* temp vertex pointer */
	float temp_color[4];
...
void CFont_3D::glfDraw3DSolidSymbolF(int font_descriptor, char s)
{
	int temp;
...
void CFont_3D::glfDraw3DSolidString(const char *s)
{
	DrawString(s, &glfDraw3DSolidSymbol);
}

/* Draw 3D solid string by font_descriptor */
void CFont_3D::glfDraw3DSolidStringF(int font_descriptor, const char *s)
{
	int temp;
...
void CFont_3D::glfGetStringBoundsF(int fd, const char *s, float *minx, float *miny, float *maxx, float *maxy)
{
	...
	if (font->symbols[(int)(s[0])])
		minxx = font->symbols[(int)(s[0])]->leftx;
	else
		minxx = 0.0;
...
void CFont_3D::glfGetStringBounds(char *s, float *minx, float *miny, float *maxx, float *maxy)
{
	glfGetStringBoundsF(curfont, s, minx, miny, maxx, maxy);
}

void CFont_3D::glfSetSymbolSpace(float sp)
{
	SymbolDist = sp;
}

float CFont_3D::glfGetSymbolSpace()
{
	return SymbolDist;
}

void CFont_3D::glfSetSpaceSize(float sp)
{
	SpaceSize = sp;
}

float CFont_3D::glfGetSpaceSize()
{
	return SpaceSize;
}

void CFont_3D::glfSetSymbolDepth(float dpth)
{
	SymbolDepth = dpth;
}

float CFont_3D::glfGetSymbolDepth()
{
	return SymbolDepth;
}

int CFont_3D::glfSetCurrentFont(int Font_Descriptor)
{
	if ((Font_Descriptor < 0) || (fonts[Font_Descriptor] == NULL)) return GLF_ERROR;
...
int CFont_3D::glfGetCurrentFont()
{
	return curfont;
}

void CFont_3D::glfSetAnchorPoint(int anchp)
{
	if ((anchp >= GLF_LEFT_UP) && (anchp <= GLF_RIGHT_DOWN))
		ap = anchp;
}

void CFont_3D::glfSetContourColor(float r, float g, float b, float a)
{
...
void CFont_3D::glfEnable(int what)
{
	switch (what) 
	{
...
void CFont_3D::glfDisable(int what)
{
	switch (what) 
	{
...
void CFont_3D::glfSetConsoleParam(int width, int height)
{
...
int CFont_3D::glfSetConsoleFont(int Font_Descriptor)
{
	if ((Font_Descriptor < 0) || (fonts[Font_Descriptor] == NULL)) return GLF_ERROR;
...
void CFont_3D::glfPrint(const char *s, int lenght)
{
	int i;
...
void CFont_3D::glfPrintString(const char *s)
{
	glfPrint(s, strlen(s));
}

void CFont_3D::glfPrintChar(char s)
{
	glfPrint(&s, 1);
}

void CFont_3D::glfConsoleDraw()
{
	int i, j;
	char s[512];
...
void CFont_3D::glfSetCursorBlinkRate(int Rate)
{
	if (Rate > 0)
	{
		conCursorBlink = Rate;
...
void CFont_3D::glfStringCentering(GLboolean center)
{
...
/* GLF_LEFT by default */
void CFont_3D::glfStringDirection(GLuint direction)
{
...
GLuint CFont_3D::glfGetStringDirection()
{
	return m_direction;
}
...
GLboolean CFont_3D::glfGetStringCentering()
{
	return m_string_center;
}

/* Set rotate angle for vector fonts */
void CFont_3D::glfSetRotateAngle(float angle)
{
	RotateAngle = angle;
}
...
void CFont_3D::bwtorgba(unsigned char *b,unsigned char *l,int n)
{
    while (n--)
    {
		l[0] = *b; l[1] = *b;
...
void CFont_3D::latorgba(unsigned char *b, unsigned char *a,unsigned char *l,int n)
{
	while(n--)
	{
		l[0] = *b; l[1] = *b;
...
void CFont_3D::rgbtorgba(unsigned char *r,unsigned char *g,unsigned char *b,unsigned char *l,int n)
{
    while(n--)
	{
		l[0] = r[0]; l[1] = g[0];
...
void CFont_3D::rgbatorgba(unsigned char *r,unsigned char *g,unsigned char *b,unsigned char *a,unsigned char *l,int n)
{
    while(n--)
	{
		l[0] = r[0]; l[1] = g[0];
...
void CFont_3D::ConvertShort(unsigned short *array, long length)
{
    unsigned b1, b2;
    unsigned char *ptr;
...
void CFont_3D::ConvertLong(unsigned *array, long length)
{
...
/* Open RGB Image */
ImageRec* CFont_3D::ImageOpen(FILE *f)
{
    union
	{
		int testWord;
    ...
    if (image == NULL)
	{
        ...
        GSLOG(SYSLOG_SEV_CRITICAL,"Out of memory! Application will now exit.");
		exit(EXIT_FAILURE);
    }
  ...
  if (fread(image, 1, 12, image->file) != 12) {
	...
	{
        GSLOG(SYSLOG_SEV_CRITICAL, "can't malloc some images : Out of memory");
		fprintf(stderr, "Out of memory!\n");
		exit(EXIT_FAILURE);
    }
    ...
    if (fread(image->rowStart, 1, x, image->file) != x) {
      //FIXME: send error
    }
    if (fread(image->rowSize, 1, x, image->file) != x) {
      //FIXME: send error
    }
		if (swapFlag)
		{
...
void CFont_3D::ImageClose(ImageRec *image)
{
	free(image->tmp);
	free(image->tmpR);
	free(image->tmpG);
...
void CFont_3D::ImageGetRow(ImageRec *image, unsigned char *buf, int y, int z)
{
    ...
    unsigned int row_size = image->rowSize[y+z*image->ysize];
    if (fread(image->tmp, 1, row_size, image->file) != row_size) {
      //FIXME: send error
    }
    ...
    if (fread(buf, 1, image->xsize, image->file) != image->xsize) {
      //FIXME: send error
    }
    }
}
...
unsigned* CFont_3D::read_texture(FILE *f, int *width, int *height, int *components)
{
    unsigned *base, *lptr;
...
unsigned* CFont_3D::texture_to_mask(unsigned* tex, int width, int height)
{
	int nSize, i;
	unsigned *ret;
...
int CFont_3D::glfLoadBMFFont(const char *FName)
{
	FILE *f;
	char Header[4];
	char FontName[97];
  ...
  if (fread(FontName, 1, 96, f) != 96) {
	...
	for (i=0; i<256; i++)
	{
    if (fread(&tx, 4, 1, f) != 1) { return GLF_ERROR; }
    if (fread(&ty, 4, 1, f) != 1) { return GLF_ERROR; }
    if (fread(&tw, 4, 1, f) != 1) { return GLF_ERROR; }
    if (fread(&th, 4, 1, f) != 1) { return GLF_ERROR; }
...
int CFont_3D::glfUnloadBMFFontD(int bmf_descriptor)
{
...
void CFont_3D::glfStopBitmapDrawing()
{
...
int CFont_3D::glfSetCurrentBMFFont(int Font_Descriptor)
{
...
int CFont_3D::glfGetCurrentBMFFont()
{
	return bmf_curfont;
}

/* Draw one bitmapped symbol by current font */
void CFont_3D::glfDrawBSymbol(char s)
{
...
void CFont_3D::glfDrawBString(const char *s)
{
	GLfloat temp_trans;
	int i;
	...
	for (i=0; i<(int)strlen(s); i++)
		temp_trans += m_widths[bmf_curfont].width[(int)s[i]] + sym_space;
	...
	if (m_bitmap_string_center == true)	glPopMatrix();
}

void CFont_3D::glfDrawBMaskSymbol(char s)
{
	if ((bmf_curfont < 0) || (bmf_in_use[bmf_curfont] == 0)) return;
...
void CFont_3D::glfDrawBMaskString(const char *s)
{
	GLfloat temp_trans;
	int i;
...
void CFont_3D::glfBitmapStringCentering(GLboolean center)
{
	m_bitmap_string_center = center;
}

/* Set string centering for bitmap fonts */
GLboolean CFont_3D::glfBitmapGetStringCentering()
{
	return m_bitmap_string_center;
}

/* Set rotate angle for bitmap fonts */
void CFont_3D::glfSetBRotateAngle(float angle)
{
	RotateAngleB = angle;
}
