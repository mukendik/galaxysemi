...
///////////////////////////////////////////////////////////////////////////////////
//
// Name			:	bool GexERProdYield::computeData()
//
// Description	:	Compute data for the report
//
///////////////////////////////////////////////////////////////////////////////////
bool GexERProdYield::computeData()
{
	// Compute Yield data for specified filters
	QString		strBinList = m_pSection->pEnterpriseReport->m_strBinList;
	QStringList	strlBinningDef;
	int			nNumber;
	int			nBinning = -1;
	bool		bOK;
	bool		bSoftBin=false;

	if(strBinList.toLower() != "pass")
	{
        strlBinningDef = strBinList.split(":");
		if(strlBinningDef.count() == 1)
		{
			nNumber = strBinList.toInt(&bOK);
			if(bOK)
				nBinning = nNumber;
		}
		else
		{
			nNumber = strlBinningDef[1].toInt(&bOK);
			if(bOK)
			{
				nBinning = nNumber;
				if(strlBinningDef[0].toLower() == "sbin")
					bSoftBin = true;
			}
		}
	}

	if (!m_pDatabaseEntry->m_pExternalDatabase->GetDataForProd_Yield(m_cFilters, m_clXYChartList, nBinning, bSoftBin))
	{
		QString strErrorMsg;
...
///////////////////////////////////////////////////////////////////////////////////
//
// Name			:	void GexERProdYield::writeRawData()
//
// Description	:	Create the raw data file
//
///////////////////////////////////////////////////////////////////////////////////
void GexERProdYield::writeRawData()
{
...
///////////////////////////////////////////////////////////////////////////////////
//
// Name			:	void GexERProdYield::writeReport()
//
// Description	:	Write the graphs and tables
//
///////////////////////////////////////////////////////////////////////////////////
void GexERProdYield::writeReport()
{
	// Create image(s)
	GexDbPlugin_XYChart_Data *	pXYChartData;
	int							nIndex;
	...
	if ((m_pSection->pEnterpriseReport->m_strLayerMode.toLower() == "multi-layers") && (m_clXYChartList.count() > 1))
	{
		...
		if(m_pSection->pEnterpriseReport->m_strChartingMode.toLower() == "bars")
			makeXYChart(m_clXYChartList, strImageFullName, "Yield (%)", 2, true);
		else
			makeXYChart(m_clXYChartList, strImageFullName, "Yield (%)", 2, false);
		...
		gexReport->SetPowerPointSlideName("Yield chart");
...
//		for(pXYChartData = m_clXYChartList.first(); pXYChartData; pXYChartData = m_clXYChartList.next())
        foreach(pXYChartData, m_clXYChartList)
		{
			...
			if(m_pSection->pEnterpriseReport->m_strLeftAxis.toLower() == "serie - yield")
			{
				...
				pXYChartData->m_strYAxisLegend = "Yield (%)";
			...
			// If creating a Powerpoint presentation, save Section title.
			gexReport->SetPowerPointSlideName("Yield chart : " + pXYChartData->m_strSplitValue);
...
///////////////////////////////////////////////////////////////////////////////////
//
// Name			:	void GexERProdYield::writeHtmlData(QString strSplitValue)
//
// Description	:	Write the raw data table
//
///////////////////////////////////////////////////////////////////////////////////
void GexERProdYield::writeHtmlData(QString strSplitValue)
{
	GexDbPlugin_XYChart_Data *	pXYChartData;
	int							nIndex;
	...
	if (strSplitValue.isEmpty())
		gexReport->SetPowerPointSlideName("Yield raw data");
	else
		gexReport->SetPowerPointSlideName("Yield raw data : " + strSplitValue);
    ...
    foreach(pXYChartData, m_clXYChartList)
	{
		if(strSplitValue.isEmpty() || (pXYChartData->m_strSplitValue == strSplitValue))
		{
			for(nIndex = 0; nIndex < pXYChartData->m_nNbDataPoints; nIndex++)
			{
				...
				// Add a new line in the Html table
