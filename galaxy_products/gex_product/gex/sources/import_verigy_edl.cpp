...
public:
  ...
  gsint32 GetStorageSizeBytes (void)		{return 0;};
...
CGVERIGY_EDLtoSTDF::CGVERIGY_EDLtoSTDF()
{
  ...
  if (!m_szBlock)
    GSLOG(SYSLOG_SEV_ERROR, "cant malloc() !");
  m_nBlockData = 0;
  m_nBlockIndex = 0;
...
//////////////////////////////////////////////////////////////////////
int CGVERIGY_EDLtoSTDF::ReadVerigyEdlFile()
{
  QString			strString;
  QString			strSection;
  QString			strValue;
  ...
  // Debug message
  strString = "---- CGVERIGY_EDLtoSTDF::ReadVerigyEdlFile(";
  strString += m_strVerigyEdlFileName;
  strString += ")";
  WriteDebugMessageFile(strString);
...
int CGVERIGY_EDLtoSTDF::ReadEventRecord(QFile* pFile)
{
  ...
  if(global_bDumpRecord)
  {
    // Debug message
    const char * event_names[] = EVENT_NAMES;
    QString strString = "---- CGVERIGY_EDLtoSTDF::ReadEventRecord(";
    strString += event_names[sWord];
    strString += ")";
    WriteDebugMessageFile(strString);
  }
...
int CGVERIGY_EDLtoSTDF::ReadAttributeRecord()
{
  ...
  {
    // Debug message
    const char * attribute_names[] = ATTRIBUTE_NAMES;
    strString = "---- CGVERIGY_EDLtoSTDF::ReadAttributeRecord(";
    if((sWord <= 0)
    || ((sWord+1) > NumberOfAttributeTypes))
    strString += "UnknownAttribute"+QString::number(sWord);
    else
      strString += attribute_names[sWord];
    strString += ")";
    WriteDebugMessageFile(strString);
  }
  ...
  case  NamedValueAttrType:             /*  7 */
    ...
    if(bByte == 1)
    {
      ...
      if(strString.toUpper() == "START_TIME") //20051213 123813
      {
        ...
        gsint32 lTime = QDateTime::fromString(szString,Qt::ISODate).toTime_t();
        if(lTime > 0)
          m_lStartTime = lTime;
      }
...
int CGVERIGY_EDLtoSTDF::WriteMir()
{
  ...
  // Write ATR
  strString = GS::Gex::Engine::GetInstance().Get("AppFullName").toString();
...
//////////////////////////////////////////////////////////////////////
int CGVERIGY_EDLtoSTDF::Convert(const char *VerigyEdlFileName, QStringList &lstFileNameSTDF, bool bAllowOnlyOneFile)
{
  ...
  if(GexScriptStatusLabel != NULL)
  {
    if(GexScriptStatusLabel->isHidden())
    {
      ...
      GS::Gex::Engine::GetInstance().UpdateLabelStatus("Converting data from file "+QFileInfo(VerigyEdlFileName).fileName()+"...");
...
int	 CGVERIGY_EDLtoSTDF::ReadBlock(QFile* pFile, char *data, quint64 len)
{
  ...
  if(GexProgressBar != NULL)
    {
        while( pFile->pos() > (qint64)iNextFilePos)
    {
