...
REM script to transfer a gexdb from a MySQL server to another one WITHOUT writing .sql file(s). 

echo Starting gexdb export: > Export_Gexdb.log
time /T >> Export_Gexdb.log
echo ... >> Export_Gexdb.log

REM Test : export to samba file. Exple : \\Jupiter\Jupiter_Shared\Gexdb_new.sql
...
set GEXDB_SOURCE=gexdb_new
set GEXDB_TARGET=gexdb_vy

set SOURCE_HOST=192.168.1.22
set TARGET_HOST=localhost

set SOURCE_USER=gexdb_new
set SOURCE_PASSWORD=gexadmin_new
set TARGET_PASSWORD=arthur66

set CREATEDB=--verbose

REM first try with a small tables : global_info wt_metadata_mapping
REM mysqldump --no-defaults -u %SOURCE_USER% -p%SOURCE_PASSWORD% --host=%SOURCE_HOST% --verbose --skip-set-charset --skip-opt --hex_blob --create-options --comments --disable-keys %CREATEDB%  --extended-insert --quick %GEXDB_SOURCE% global_info wt_metadata_mapping | mysql -u root -parthur66 --host=%TARGET_HOST% --compress --database=%GEXDB_TARGET%  
...
echo gexdb export finished >> Export_Gexdb.log
time /T >> Export_Gexdb.log
pause
