...
// Constructor
///////////////////////////////////////////////////////////
ReadSystemInfo::ReadSystemInfo(void)
{
    // Write debug info (if Debug mode enabled)
    GSLOG(7, "ReadSystemInfo constructor ");
    ...
    iErrorCode = ReadBasicSystemInfo();
    if(iErrorCode != READ_SYS_INFO_NOERROR)
    {
        GSLOG(3, "ReadBasicSystemInfo failed");
        return;
    }
    // Get PC-Host ID : built from Serial# of computer disks...if no disk #, value remains 0.
    iErrorCode = ReadPcHostID();
    ...
    iErrorCode = ReadNetworkBoardInfo();
}
...
//	strPlatform = "[PC-Windows]";
    ...
    if(lRes != ERROR_SUCCESS)
    {
        GSLOG(3, "failed to read hostname with RegOpenKey !");
        return READ_SYS_INFO_ERROR_HOSTNAME;	// ERROR: Failed reading hostname
...
#   if defined(_WIN32) || defined(_WIN64) || defined(__WIN32__)
    ...
    // Trace message
    strTraceMessage = QString::number(iCount) + " adapters detected";
    GSLOG(7, strTraceMessage.toLatin1().data() );
...
check_net_adaptors:
   ...
   GetAdaptersInfo = NULL; /* pointer to function */
    ...
    // Trace message
    strTraceMessage = "Adapters decoded (iphlpapi.dll) :" + strNetworkBoardsIDs;
    GSLOG(7, strTraceMessage.toLatin1().data() );
...
#       endif
        ...
        QString mac = QString("%1:%2:%3:%4:%5:%6").
            arg(ptr[0], 2, 16, QChar('0')).arg(ptr[1], 2, 16, QChar('0')).
            arg(ptr[2], 2, 16, QChar('0')).arg(ptr[3], 2, 16, QChar('0')).
            arg(ptr[4], 2, 16, QChar('0')).arg(ptr[5], 2, 16, QChar('0'));
        if (strNetworkBoardID.isEmpty())
        {
            strNetworkBoardID = mac;
        }
        strNetworkBoardsIDs += mac;
        strNetworkBoardsIDs += ";";
    }

    freeifaddrs(ifap);
#   endif
    GSLOG(SYSLOG_SEV_DEBUG, QString("NetworkBoardsIDs=%1").
          arg(strNetworkBoardsIDs).toLatin1().constData());
    return READ_SYS_INFO_NOERROR;
}
...
//           "Error", "ERROR: Could not open IDE21201.VXD file");
//       GS::Gex::Engine::GetInstance().Exit(EXIT_FAILURE);
       GSLOG(SYSLOG_SEV_CRITICAL, "ERROR: Could not open IDE21201.VXD file");
       return false;
   }
